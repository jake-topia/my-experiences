class CTFGameManager extends SystemScript {
  _destroyed: boolean;
  _hostPlayerId: number;
  _redScore: number; // simple numeric score
  _blueScore: number; // simple numeric score
  _spawnPoints: PseudoMap; // team -> spawn { x, y }
  _currentState: string;
  _countdownEndTime: number;
  _winnerTeam: string;
  _isPhysicsRunning: boolean;
  // single neutral flag sprite id removed (dual only)
  _logEnabled: boolean;
  _countdownStarted: boolean;
  _finalScoresComputed: boolean;
  _lastHudUpdateT: number;
  _stageWidth: number;
  _stageHeight: number;
  _stageSize: number; // min(width,height) for square logic
  _cellSize: number; // stageSize / 3 for 3x3 grid
  _redSpawnX: number;
  _blueSpawnX: number;
  _centerY: number;
  _configProvider: any;
  _cfg: PseudoMap;
  _versionTag: string;
  _teamRedList: PseudoList;
  _teamBlueList: PseudoList;
  // multi-team structures
  _teams: PseudoMap; // team -> PseudoList
  _teamOrder: PseudoList; // order of teams
  _teamEmoji: PseudoMap; // team -> emoji
  _teamColors: PseudoMap; // team -> territory color hex
  _playerColors: PseudoMap; // team -> player tint color hex (lighter versions)
  _armByTeam: PseudoMap; // team -> 'N'|'W'|'E'|'S'
  _neutralCells: PseudoList; // list of { r, c }
  _gridRects: PseudoMap; // 'r{row}c{col}' -> rect
  // dual-flag only mode (always true conceptually)
  _flagStates: PseudoMap; // team -> { spriteId,pos:{x,y},heldBy,atBase }
  _carrierOfFlagTeam: PseudoMap; // team -> pid
  _flagHeld: PseudoMap; // team -> boolean
  _scores: PseudoMap; // team -> number
  _physicsBuf: number;
  _pendingCaptureBlockedT: number;
  _redCarrierId: number;
  _blueCarrierId: number;
  _redFlagHeld: boolean; // simple booleans for fast re-trigger guards
  _blueFlagHeld: boolean;
  _recentTeleport: PseudoMap; // pidStr -> timestamp (ms)
  _spawnImmunity: PseudoMap; // pidStr -> expiry timestamp (ms)
  // anti-camping system
  _flagZoneSprites: PseudoMap; // team -> spriteId (semi-opaque white rectangle)
  _campingTimers: PseudoMap; // pidStr -> timestamp when camping started (ms)
  _flagZoneRadius: number; // radius around flag for camping detection
  // analytics
  _analyticsPublicKey: string;
  // notification system
  _notificationText: string;
  _notificationExpiry: number;
  // debug helpers
  _debugAuditOnceAfterStart: boolean;
  ...
  onInit() {
    ...
    this._redSpawnX = Math.round(
      this._stageWidth * (this._cfg.redSpawnXFactor || 0.125),
    );
    this._blueSpawnX = Math.round(
      this._stageWidth * (this._cfg.blueSpawnXFactor || 0.875),
    );
    this._centerY = Math.round(
      this._stageHeight * (this._cfg.spawnYFactor || 0.5),
    );
    // initialize but will be recomputed by computeLayout()
    this._spawnPoints = {} as PseudoMap;
    this._spawnPoints.red = { x: this._redSpawnX, y: this._centerY } as any;
    this._spawnPoints.blue = { x: this._blueSpawnX, y: this._centerY } as any;
    ...
    this._recentTeleport = {} as PseudoMap;
    this._spawnImmunity = {} as PseudoMap;
    ...
    this._teams = {} as PseudoMap;
    this._teams.red = [];
    this._teams.blue = [];
    this._teams.green = [];
    this._teams.yellow = [];
    this._teamOrder = [];
    (this._teamOrder as any).push('red');
    (this._teamOrder as any).push('blue');
    (this._teamOrder as any).push('green');
    (this._teamOrder as any).push('yellow');
    this._teamEmoji = {} as PseudoMap;
    this._teamEmoji.red = '🔴' as any;
    this._teamEmoji.blue = '🔵' as any;
    this._teamEmoji.green = '🟢' as any;
    this._teamEmoji.yellow = '🟡' as any;

    // Territory colors (darker, saturated)
    this._teamColors = {} as PseudoMap;
    this._teamColors.red = '#3B0200' as any; // Dark red
    this._teamColors.blue = '#0A2D43' as any; // Dark blue
    this._teamColors.green = '#184208' as any; // Dark green
    this._teamColors.yellow = '#BA7915' as any; // Dark yellow/gold

    // Player tint colors (lighter versions for visibility)
    this._playerColors = {} as PseudoMap;
    this._playerColors.red = '#D9A19A' as any; // Light red
    this._playerColors.blue = '#A3C4D9' as any; // Light blue
    this._playerColors.green = '#A8D9A1' as any; // Light green
    this._playerColors.yellow = '#F5D9A3' as any; // Light yellow/gold

    this._armByTeam = {} as PseudoMap;
    this._armByTeam.red = '' as any;
    this._armByTeam.blue = '' as any;
    this._armByTeam.green = '' as any;
    this._armByTeam.yellow = '' as any;
    this._neutralCells = [];
    (this._neutralCells as any).push({ r: 0, c: 0 });
    (this._neutralCells as any).push({ r: 0, c: 2 });
    (this._neutralCells as any).push({ r: 1, c: 1 });
    (this._neutralCells as any).push({ r: 2, c: 0 });
    (this._neutralCells as any).push({ r: 2, c: 2 });
    this._gridRects = {} as PseudoMap;
    this._carrierOfFlagTeam = {} as PseudoMap;
    this._carrierOfFlagTeam.red = 0 as any;
    this._carrierOfFlagTeam.blue = 0 as any;
    this._carrierOfFlagTeam.green = 0 as any;
    this._carrierOfFlagTeam.yellow = 0 as any;
    this._flagHeld = {} as PseudoMap;
    this._flagHeld.red = false as any;
    this._flagHeld.blue = false as any;
    this._flagHeld.green = false as any;
    this._flagHeld.yellow = false as any;
    this._scores = {} as PseudoMap;
    this._scores.red = 0 as any;
    this._scores.blue = 0 as any;
    this._scores.green = 0 as any;
    this._scores.yellow = 0 as any;
    this._physicsBuf = 0;
    ...
    // Anti-camping system
    this._flagZoneSprites = {} as PseudoMap;
    this._campingTimers = {} as PseudoMap;
  ...
  log(m: string) {
    if (this._logEnabled) console.log('[CTF] ' + m);
  }

  showNotification(msg: string, durationMs: number) {
  ...
  _debugAudit(label: string) {
    if (!this._logEnabled) return;
    try {
      const teams = {
        red: (this._teamRedList as any).length,
        blue: (this._teamBlueList as any).length,
        green: ((this._teams.green as any) || []).length,
        yellow: ((this._teams.yellow as any) || []).length,
      } as any;
      const arms: any = {};
      const spawns: any = {};
  ...
  _getTeam(pid: number): string {
    const r = this._teamRedList as any;
    for (let i = 0; i < r.length; i++) if (r[i] === pid) return 'red';
    const b = this._teamBlueList as any;
    for (let i = 0; i < b.length; i++) if (b[i] === pid) return 'blue';
    const g = (this._teams.green as any) || [];
    for (let i = 0; i < g.length; i++) if (g[i] === pid) return 'green';
    const y = (this._teams.yellow as any) || [];
    for (let i = 0; i < y.length; i++) if (y[i] === pid) return 'yellow';
    return '';
  }
  _playerCarriesFlag(pid: number): string {
    // generalized: return the team whose flag this player carries (enemy team key)
    for (let i = 0; i < this._teamOrder.length; i++) {
      const team = this._teamOrder[i];
      if (this._carrierOfFlagTeam[team] === pid) return team;
    }
    // legacy fallback
    if (this._redCarrierId === pid) return 'blue';
    if (this._blueCarrierId === pid) return 'red';
    return '';
  }

  _playerCarriesAllFlags(pid: number): string[] {
  ...
  _clearCarrierForFlag(flagTeam: string) {
    if (flagTeam === 'red') this._blueCarrierId = 0; // blue player was carrying red flag
    if (flagTeam === 'blue') this._redCarrierId = 0; // red player was carrying blue flag
  }

  // COUNT HELPERS
  _count(team: string) {
    if (team === 'red') return (this._teamRedList as any).length;
    if (team === 'blue') return (this._teamBlueList as any).length;
    const list = this._teams[team];
    return list ? (list as any).length : 0;
  }
  _list(team: string) {
    ...
    const src =
      team === 'red'
        ? (this._teamRedList as any)
        : team === 'blue'
          ? (this._teamBlueList as any)
          : (this._teams[team] as any) || ([] as any);
    const out: number[] = [];
  ...
  computeLayout() {
    ...
    if (teamCount === 2) {
      ...
      this._armByTeam[order[0]] = 'W' as any;
      this._armByTeam[order[1]] = 'E' as any;
      ...
      this._armByTeam[order[0]] = 'W' as any; // Left third
      this._armByTeam[order[1]] = 'N' as any; // Middle third (was center, now equal size)
      this._armByTeam[order[2]] = 'E' as any; // Right third
      ...
      this._armByTeam[order[0]] = 'NW' as any;
      this._armByTeam[order[1]] = 'NE' as any;
      this._armByTeam[order[2]] = 'SW' as any;
      this._armByTeam[order[3]] = 'SE' as any;
    ...
    for (let ti = 0; ti < order.length; ti++) {
      ...
      if (arm === 'W') spawnX = flagX - offset;
      else if (arm === 'E') spawnX = flagX + offset;
      else if (arm === 'N') spawnY = flagY - offset;
      else if (arm === 'S') spawnY = flagY + offset;
      else if (arm === 'NW') {
        spawnX = flagX - offset;
        spawnY = flagY - offset;
      } else if (arm === 'NE') {
  ...
  rectForArm(arm: string) {
    // Return base rect for arm - now uses larger fair rectangles
    const baseKey = 'base' + arm;
    if (this._gridRects[baseKey]) return this._gridRects[baseKey];

    // Fallback for old arm keys (compatibility with neutral zone checks)
    if (arm === 'N') return this._gridRects['r0c1'];
    if (arm === 'W') return this._gridRects['r1c0'];
    if (arm === 'E') return this._gridRects['r1c2'];
    if (arm === 'S') return this._gridRects['r2c1'];
  ...
  whichArm(x: number, y: number): string {
    // Check new base rectangles first (larger fair zones)
    const basePairs: { arm: string; key: string }[] = [
      { arm: 'W', key: 'baseW' },
      { arm: 'E', key: 'baseE' },
      { arm: 'N', key: 'baseN' },
      { arm: 'NW', key: 'baseNW' },
    ...
    const pairs: { arm: string; key: string }[] = [
      { arm: 'N', key: 'r0c1' },
      { arm: 'W', key: 'r1c0' },
      { arm: 'E', key: 'r1c2' },
      { arm: 'S', key: 'r2c1' },
  ...
  onPlayerJoined(o: { playerId: number }) {
    if (!playerManager.isHost) return;
  ...
  onPlayerLeft(o: { playerId: number }) {
    if (!playerManager.isHost) return;
    const pid = o.playerId;
    if (!pid) return;
    this.log('Player left P' + pid);
    // remove from lists (including green and yellow teams)
    function remove(list: any[], v: number) {
      for (let i = 0; i < list.length; i++) {
        if (list[i] === v) {
          for (let j = i + 1; j < list.length; j++) list[j - 1] = list[j];
          list.pop();
          break;
        }
      }
    }
    remove(this._teamRedList as any, pid);
    remove(this._teamBlueList as any, pid);
    remove(this._teams.green as any, pid);
    remove(this._teams.yellow as any, pid);
  ...
  assignPlayerToTeam(pid: number, team: string) {
    if (
      team !== 'red' &&
      team !== 'blue' &&
      team !== 'green' &&
    ...
    if (this._getTeam(pid) === team) return true;
    // helper indexOf (no ES6 assumed)
    function idx(list: any[], v: number) {
      for (let i = 0; i < list.length; i++) {
    ...
    this.log(
      'Assign P' +
        ...
        team +
        ' (R=' +
        (this._teamRedList as any).length +
        ' B=' +
        (this._teamBlueList as any).length +
        ') tint=' +
        playerTint,
    );
    this.computeLayout();
  ...
  _formatNameplate(pid: number, baseText: string): string {
  ...
  drawOrUpdateTeamSelect() {
    if (!playerManager.isHost) return;
    const redBtn = 'ctf_team_red_btn',
      blueBtn = 'ctf_team_blue_btn',
      greenBtn = 'ctf_team_green_btn',
      yellowBtn = 'ctf_team_yellow_btn',
      startBtn = 'ctf_start_btn',
      selectTeamsHeader = 'ctf_select_teams_header';
    const rc = (this._teamRedList as any).length;
    const bc = (this._teamBlueList as any).length;
    const gc = (this._teams.green as any).length;
    const yc = (this._teams.yellow as any).length;
    const max = this._cfg.maxPlayersPerTeam || 4;
    // start requires at least 2 active teams
    const activeTeams =
      (rc > 0 ? 1 : 0) + (bc > 0 ? 1 : 0) + (gc > 0 ? 1 : 0) + (yc > 0 ? 1 : 0);
    const showStart =
      activeTeams >= 2 &&
    ...
    const allowSwitch =
      this._cfg.allowTeamSwitchDuringCountdown !== false ||
      !this._countdownStarted;
    ...
    if (this._currentState !== 'TEAM_SELECT') {
      ...
      for (let i = 0; i < arr.length; i++) {
        if (spriteManager.getSprite(arr[i])) spriteManager.removeSprite(arr[i]);
      }
      return;
    }
    ...
    const instructionText =
      '🚩 Capture the Flag! Grab the enemy flag and return it to your base. First to 3 wins! 🏆';
    ...
    if (spriteManager.getSprite(redBtn)) spriteManager.removeSprite(redBtn);
    ...
    if (spriteManager.getSprite(blueBtn)) spriteManager.removeSprite(blueBtn);
    ...
    if (spriteManager.getSprite(greenBtn)) spriteManager.removeSprite(greenBtn);
  ...
  onEvent_playerSelectsTeam(eventData: { playerId: number; team: string }) {
    if (!playerManager.isHost) return;
    if (!eventData || !eventData.playerId) return;
  ...
  onEvent_hostStartsGame(eventData: { playerId: number }) {
    if (!playerManager.isHost) return;
    if (this._currentState !== 'TEAM_SELECT' || this._countdownStarted) return;
    if (!eventData || !eventData.playerId) return;
    this.log('Start requested by P' + eventData.playerId);
    this.startCountdown();
  }
  // Alias for emitted 'startGame'
  onEvent_startGame(eventData: { playerId: number }) {
    this.onEvent_hostStartsGame(eventData);
  }

  onEvent_restartGame(eventData: { playerId: number }) {
    if (!playerManager.isHost) return;
    if (this._currentState !== 'GAME_OVER') return;
    if (!eventData || !eventData.playerId) return;
  ...
  startCountdown() {
    ...
    for (let i = 0; i < ids.length; i++) {
      if (spriteManager.getSprite(ids[i])) spriteManager.removeSprite(ids[i]);
    }
    this.updateHud();
  }
  ...
  _debugCheckRecentTeleport() {
    ...
    try {
      for (const k in this._recentTeleport) {
        const v: any = (this._recentTeleport as any)[k];
        if (typeof v !== 'number' || !isFinite(v)) {
          this.log(
            'TP_BAD key=' + k + ' vType=' + this._typeOf(v) + ' val=' + v,
          );
          // sanitize: remove bad entry so serializer never sees undefined
          delete (this._recentTeleport as any)[k];
        }
      }
    } catch (e) {
  ...
  _debugCheckAllStates() {
    ...
    try {
      ...
      for (const k in this._carrierOfFlagTeam) {
        const v: any = (this._carrierOfFlagTeam as any)[k];
        if (typeof v !== 'number' || !isFinite(v)) {
          this.log(
            'CARRIER_BAD key=' + k + ' vType=' + this._typeOf(v) + ' val=' + v,
          );
          (this._carrierOfFlagTeam as any)[k] = 0;
        }
      }

      for (const k in this._flagHeld) {
        const v: any = (this._flagHeld as any)[k];
        if (typeof v !== 'boolean') {
          this.log(
            'FLAGHELD_BAD key=' + k + ' vType=' + this._typeOf(v) + ' val=' + v,
          );
          (this._flagHeld as any)[k] = false;
        }
      }

      for (const k in this._scores) {
        const v: any = (this._scores as any)[k];
        if (typeof v !== 'number' || !isFinite(v)) {
          this.log(
            'SCORE_BAD key=' + k + ' vType=' + this._typeOf(v) + ' val=' + v,
          );
          (this._scores as any)[k] = 0;
        }
      }
    } catch (e) {
  ...
  update() {
    if (!playerManager.isHost || this._destroyed) return;
    const now = Date.now();
    if (
      this._currentState === 'PRE_COUNTDOWN' &&
  ...
  startMatch() {
    ...
    for (let i = 0; i < this._teamOrder.length; i++) {
      const t = this._teamOrder[i];
      if (t !== 'red' && t !== 'blue') this._scores[t] = 0;
      this._carrierOfFlagTeam[t] = 0;
      this._flagHeld[t] = false;
    }
    this._winnerTeam = '';
    ...
    for (var ti = 0; ti < teams.length; ti++) {
      ...
      for (var k = 0; k < arr.length; k++) this.teleportToSpawn(arr[k], tkey);
    }
  ...
  spawnDualFlags() {
    try {
      ...
      for (let i = 0; i < this._teamOrder.length; i++) {
        ...
        if (!isActive) {
          if (spriteManager.getSprite(id)) spriteManager.removeSprite(id);
  ...
  teleportToSpawn(pid: number, team: string) {
  ...
  onSpriteCollisionStart({
    ...
    if (playerSprite && flagSprite) {
      ...
      if (pid) {
        ...
        if (idx !== -1) ownerTeam = uid.substring(idx + pfx.length);
        ...
        if (ownerTeam) {
          ...
          if (fs && fs.atBase && fs.heldBy === 0 && playerTeam !== ownerTeam) {
            ...
            if (this._flagHeld[ownerTeam]) return;
            fs.atBase = false;
            fs.heldBy = pid;
            ...
            playerManager.setNameplate(
              pid,
              this._formatNameplate(pid,
                (this._teamEmoji[playerTeam] || '') +
                  ' ' +
                  flagIcons +
                  ' ' +
                  base)
            ...
            const ownerLabel =
              ownerTeam.charAt(0).toUpperCase() + ownerTeam.slice(1);
            const playerLabel =
              playerTeam.charAt(0).toUpperCase() + playerTeam.slice(1);
            this.showNotification(
              playerLabel + ' team captured the ' + ownerLabel + ' flag!',
              2500,
            );
            return;
          }
  ...
  addFlagIndicator(pid: number, flagTeam: string) {
    ...
    const offsetX =
      flagIndex * spacing - ((carriedFlags.length - 1) * spacing) / 2;
  ...
  removeFlagIndicator(pid: number, flagTeam: string) {
    const indicatorId = 'flag_indicator_' + pid + '_' + flagTeam;
  ...
  restoreNameplate(pid: number, team: string) {
    const d = playerManager.getPlayerDetails(pid);
    const base = d && d.username ? d.username : 'P' + pid;
    const emo = this._teamEmoji[team] || '';
    playerManager.setNameplate(pid, this._formatNameplate(pid, emo + ' ' + base));
  ...
  handleDualTag(pida: number, pidb: number) {
  ...
  teamForArm(arm: string): string {
    if (!arm) return '';
    for (let i = 0; i < this._teamOrder.length; i++) {
      const t = this._teamOrder[i];
      if (this._armByTeam[t] === arm && this._count(t) > 0) return t;
    }
    return '';
  }

  resetStolenFlag(carrierPid: number, flagTeam: string) {
    if (!flagTeam) return;
    ...
    this.showNotification(
      teamLabel + ' carrier tagged! ' + flagLabel + ' flag returned.',
      2000,
    );
    this.updateHud();
  }
      ...
      if (this._campingTimers[pidStr] && this._campingTimers[pidStr] > 0) {
        ...
        if (campingDuration >= campingTimeoutMs) {
          ...
          if (flagPos) {
            ...
            this.showNotification(
              playerTeam.charAt(0).toUpperCase() + playerTeam.slice(1) + ' player stop camping!',
              2000
            );
  ...
  onPhysicsStep() {
    ...
    if (!playerManager.isHost) return;
    if (this._isPhysicsRunning) return;
  ...
  scoreCaptureDual(pid: number, scoringTeam: string, carriedFlagTeam: string) {
    // guard: ensure flag marked as currently held before scoring to avoid duplicate triggers
    if (!this._flagHeld[carriedFlagTeam]) {
      this.log(
        'SCORE_GUARD: Flag not held, ignoring duplicate score attempt for ' +
          carriedFlagTeam,
      );
      return;
    }
    ...
    if (scoringTeam === 'red') this._redScore += 1;
    else if (scoringTeam === 'blue') this._blueScore += 1;
    else this._scores[scoringTeam] = (this._scores[scoringTeam] || 0) + 1;
    ...
    const scoringLabel =
      scoringTeam.charAt(0).toUpperCase() + scoringTeam.slice(1);
    const carriedLabel =
      carriedFlagTeam.charAt(0).toUpperCase() + carriedFlagTeam.slice(1);
    this.showNotification(
      scoringLabel + ' scores! Captured ' + carriedLabel + ' flag!',
      3000,
    );
    this.log(
      'Capture (dual) team=' +
  ...
  addOrUpdateZones() {
    ...
    const neutralAlpha = Math.max(
      ...
      Math.min(
        1,
        this._cfg.neutralZoneAlpha != null ? this._cfg.neutralZoneAlpha : 0.35,
      ),
    );
    ...
    for (let i = 0; i < neutralKeys.length; i++) {
      ...
      if (!rect) {
        ...
        if (spriteManager.getSprite(zId)) spriteManager.removeSprite(zId);
        continue;
      }
      this.log(
        'ZONE_NEUTRAL_LANE key=' +
          key +
          ' rect={' +
          rect.x +
          ',' +
  ...
  clearAllZones() {
    ...
    for (let i = 0; i < this._teamOrder.length; i++) {
      ...
      if (spriteManager.getSprite(flagId)) spriteManager.removeSprite(flagId);
      if (spriteManager.getSprite(flagZoneId)) spriteManager.removeSprite(flagZoneId);
    }
    ...
    try {
      ...
      for (let i = 0; i < pids.length; i++) {
        ...
        for (let j = 0; j < this._teamOrder.length; j++) {
          ...
          if (spriteManager.getSprite(indicatorId)) spriteManager.removeSprite(indicatorId);
        }
      }
    } catch (e) {
      // Non-fatal - continue
  ...
  isInZone(x: number, y: number, team: string) {
    ...
    // legacy fallback
    const half = this._stageWidth / 2;
    if (team === 'red') return x < half;
    if (team === 'blue') return x >= half;
    return false;
  }
  ...
  updateHud() {
    ...
    const timerId = 'ctf_hud_timer',
      scoreId = 'ctf_hud_scores',
      overId = 'ctf_game_over_banner';
    let t = '';
    ...
    if (!t && this._currentState === 'PRE_COUNTDOWN') {
      const sec = Math.max(
        0,
        Math.floor((this._countdownEndTime - now) / 1000),
      );
      t = 'Start in ' + sec + 's';
    ...
    if (
      ...
      if (spriteManager.getSprite(scoreId)) spriteManager.removeSprite(scoreId);
    }
    if (this._currentState === 'GAME_OVER') {
      const centerX = this._stageWidth / 2;

      // Hide countdown/timer during game over
      if (spriteManager.getSprite(timerId)) spriteManager.removeSprite(timerId);
  ...
  finalizeScores() {
    if (this._finalScoresComputed) return;
  ...
  _setWorldActivity(type: string) {
    try {
      if (!playerManager.isHost) return;
      if (!this._analyticsPublicKey) return;
