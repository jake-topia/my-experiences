class CollisionManager extends SystemScript {
  // --- Properties  ---
  colliderAssetKey: string;
  colliderComponentScriptId: string;
  colliderUniqueId: string;
  players: PseudoList;
  colliders: PseudoList;
  buffer: number;
  constructor() {
    // load state (no async!)
  ...
  addPlayer(playerId: number) {
    console.log(
      '!!!!!!!!!!!!!!!!!! Adding a collider to player number: ' + playerId,
    );
    this.spawnCollider(playerId);
  }
  /** spawnPellet Method */
  spawnCollider(playerId: number) {
    console.log(
      `CollisionManager: Attempting to add collider for player: ` + playerId,
    );
    ...
    try {
      ...
      if (colliderSprite) {
        console.log(
          'CollisionManager: addSprite successful. Attaching component...',
        );
        ...
        console.log(
          'CollisionManager: Component attached and reference stored.',
        );
      } else {
      ...
      console.log(
        `!!! CollisionManager CRITICAL ERROR during collider creation:`,
        error,
      );
    }
  ...
  onPhysicsStep() {
    ...
    if (this.buffer % 3 === 0) {
      for (let i = 0; i < this.players.length; i++) {
        ...
        if (!playerCheck.includes(this.players[i])) return;
