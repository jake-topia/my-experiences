class TimerManager extends SystemScript {
  ...
  private gameplayUpdateIntervalTargetStep: number; // For live score
  private clearTeleportFlagTimerTargetStep: number; // Add property for this timer
  private clearTeleportFlagPayload: string; // Add property for its payload
  private resetLastTeleportedTimerTargetStep: number;
  private resetLastTeleportedPayload: string; // Stores {playerId: number} as string
  ...
  onInit() {
    ...
    try {
      ...
      console.log(
        '!!! TimerManager ERROR emitting timerManagerReady event:',
        e,
      );
    }
  }
  ...
  onStep() {
    ...
    if (
      ...
      let valueToEmit: number | null = null; // Note: +"" results in 0, not NaN
      ...
      try {
        ...
        if (payloadString !== '') {
          ...
          console.log(
            `>>> Value after conversion: ${valueToEmit} (Type: ${typeof valueToEmit})`,
          ); // Optional debug log
        } else {
          console.log(
            `!!! TimerManager WARN: Stored countdownPayload was empty when firing timer.`,
          );
          valueToEmit = null;
        }
        ...
        console.log(
          `TimerManager onStep: Emitted 'handleCountdownTick' with value: ${valueToEmit}`,
        );
      } catch (e) {
        console.log(
          '!!! TimerManager onStep ERROR parsing/emitting Countdown:',
          e,
        );
        try {
    ...
    if (
      ...
      try {
        ...
        console.log(
          '!!! TimerManager onStep ERROR emitting GameplayUpdate:',
          e,
        );
      }
    ...
    if (
      this.clearTeleportFlagTimerTargetStep > 0 &&
      ...
      try {
        if (this.clearTeleportFlagPayload !== '') {
          ...
          if (!payloadToEmit || typeof payloadToEmit.playerId !== 'number') {
            console.log(
              '!!! TimerManager WARN: Invalid payload for clearTeleportFlag:',
              payloadToEmit,
            );
          ...
          console.log(
            '!!! TimerManager WARN: Empty payload for clearTeleportFlag',
          );
        }
        const playerCheck = playerManager.getPlayerIds();
          if (!playerCheck.includes(payloadToEmit.playerId)) return;

        eventManager.emit('clearTeleportFlag', payloadToEmit || {}); // Emit parsed payload or empty object
        console.log(
          `TimerManager onStep: Emitted 'clearTeleportFlag' with payload:`,
          payloadToEmit,
        );
      } catch (e) {
        console.log(
          '!!! TimerManager onStep ERROR parsing/emitting clearTeleportFlag:',
          e,
        );
      }
      ...
      console.log(
        `TimerManager onStep: Deactivated 'clearTeleportFlag' timer.`,
      );
    }
    if (
      this.resetLastTeleportedTimerTargetStep > 0 &&
      ...
      try {
        if (this.resetLastTeleportedPayload !== '') {
          ...
          if (!payloadToEmit || typeof payloadToEmit.playerId !== 'number') {
            console.log(
              '!!! TimerManager WARN: Invalid payload for resetLastTeleported:',
              payloadToEmit,
            );
            payloadToEmit = null;
          }
        ...
        console.log(
          `TimerManager onStep: Emitted 'resetLastTeleported' with payload:`,
          payloadToEmit,
        );
      } catch (e) {
        console.log(
          '!!! TimerManager onStep ERROR parsing/emitting resetLastTeleported:',
          e,
        );
      }
      ...
      this.resetLastTeleportedPayload = '';
      console.log(
        `TimerManager onStep: Deactivated 'resetLastTeleported' timer.`,
      );
    }
  }
  ...
  setTimer(
    ...
    type: 'countdown' | 'startGameplay' | 'nextLevel' | 'resetLastTeleported',
    delaySeconds: number,
    payload?: PseudoAny,
  ) {
    ...
    switch (type) {
      case 'countdown':
        ...
        try {
          ...
          if (payload && typeof payload.countdownNextValue === 'number') {
            ...
            this.countdownPayload = '';
            console.log(
              '!!! TimerManager setTimer WARN: Invalid or missing payload for countdown.',
            );
          }
        } catch (e) {
          console.log(
            '!!! TimerManager setTimer ERROR storing countdown payload:',
            e,
          );
          this.countdownPayload = '';
        }
        console.log(
          `TimerManager: Set timer 'countdown' for step ${this.countdownTimerTargetStep} with payload string '${this.countdownPayload}'`,
        );
        break;
      case 'startGameplay':
        this.startGameplayTimerTargetStep = targetStep;
        console.log(
          `TimerManager: Set timer 'startGameplay' for step ${targetStep}`,
        );
        break;
      case 'nextLevel':
        this.nextLevelTimerTargetStep = targetStep;
        console.log(
          `TimerManager: Set timer 'nextLevel' for step ${targetStep}`,
        );
        break;
      case 'resetLastTeleported':
        ...
        try {
          ...
          const playerCheck = playerManager.getPlayerIds();
          if (!playerCheck.includes(payload.playerId)) return;
          ...
          if (!this.resetLastTeleportedPayload) {
            console.log(
              '!!! TimerManager setTimer WARN: Missing payload for resetLastTeleported.',
            );
          }
        } catch (e) {
          console.log(
            '!!! TimerManager setTimer ERROR storing resetLastTeleported payload:',
            e,
          );
          this.resetLastTeleportedPayload = '';
        }
      ...
      default:
        console.log(
          `!!! TimerManager setTimer ERROR: Unknown timer type '${type}'`,
        );
    }
  }
  ...
  setInterval(type: 'gameplayUpdate', intervalSeconds: number) {
    if (type !== 'gameplayUpdate') {
      console.log(
        `!!! TimerManager setInterval ERROR: Unknown interval type '${type}'`,
      );
      return;
    }
    ...
    console.log(
      `TimerManager: Set interval 'gameplayUpdate' for initial step ${this.gameplayUpdateIntervalTargetStep}, repeating every ${intervalSteps} steps`,
    );
  }
  ...
  clearTimer(
    ...
    switch (type) {
      ...
      default:
        console.log(
          `!!! TimerManager clearTimer ERROR: Unknown timer type '${type}'`,
        );
    }
  }
