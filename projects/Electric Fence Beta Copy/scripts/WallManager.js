...
class WallManager extends SystemScript {
    ...
    startingLevel;
    // State
    activeWallSprites; // Array to track spawned walls
    ...
    extremeLevel4Layout;
    extremeLevel5Layout;
    impossibleLevel1Layout;
    impossibleLevel2Layout;
    impossibleLevel3Layout;
    impossibleLevel4Layout;
    impossibleLevel5Layout;
    blockLayout;
    possibleWalls;
    ...
    onInit() {
        ...
        try {
            ...
            console.log('!!! WallManager ERROR initializing level layouts in onInit:', e);
            ...
            for (let i = numSprites - 1; i >= 0; i--) {
                ...
                if (sprite && sprite.uniqueId) {
                    ...
                    try {
                        ...
                        console.log(`!!! WallManager ERROR removing wall ${sprite.uniqueId}:`, e);
                    }
                }
            }
        }
    ...
    loadLevel(levelIndex) {
        ...
        let layout = null; // Local variable to hold selected layout
        ...
        if (!layout || !Array.isArray(layout) || layout.length === 0) {
            console.log(`!!! WallManager ERROR: Layout data for level ${levelIndex} is missing or invalid.`);
            return;
        }
    ...
    spawnWallsFromLayout(layout, levelIndex) {
        // Ensure configuration properties are valid
        if (typeof this.wallThickness !== 'number' || !this.wallScriptId) {
            console.log('!!! WallManager ERROR: Config properties missing in spawnWallsFromLayout.');
            return;
        }
        // Layout validity already checked in loadLevel
        console.log(`--- WallManager: Processing layout with ${layout.length} rows.`);
        let spawnedCount = 0;
        for (let y = 0; y < layout.length; y++) {
            ...
            for (let x = 0; x < row.length; x++) {
                if (row[x] === 'W') {
                    ...
                    try {
                        const wallSprite = spriteManager.addSprite(this.possibleWalls[wallType], {
                            uniqueId: wallId,
                            positionX: positionX,
                            positionY: positionY,
                        ...
                        if (wallSprite) {
                            ...
                            console.log(`!!! WallManager WARNING: Failed to add sprite for wall ${wallId}`);
                        }
                    }
                    catch (e) {
                        console.log(`!!! WallManager ERROR adding sprite or attaching component for wall ${wallId}:`, e);
                    }
                }
                else if (row[x] === 'P') {
                    const positionX = x * this.wallThickness;
        ...
        console.log(`--- WallManager: Finished spawning walls for level ${levelIndex}. Spawned: ${spawnedCount} ---`);
    }
    onSpriteCollisionStart({ collisionX, collisionY, sprite1, sprite2, }) {
        ...
        try {
            ...
            console.log(`!!! Wall ${wallSprite?.uniqueId} ERROR emitting playerHitWall event for player ${tempComponent.playerId}:`, error);
        }
    }
