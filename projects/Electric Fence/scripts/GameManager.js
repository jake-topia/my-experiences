...
class GameManager extends SystemScript {
            ...
            const analytics = [
                {
                    ...
                    profileId: playerManager.getPlayerDetails(playerManager.getPlayerIds()[0]).profileId,
                    uniqueKey: playerManager.getPlayerDetails(playerManager.getPlayerIds()[0]).profileId,
                },
            ];
    ...
    clearDifficultySprites() {
        ...
        console.log('GameManager clearDifficultySprites (Analytics): Registering difficulty.');
        const level = levelMap[this.currentLevelIndex] ?? 'DefaultLevel'; // The '??' provides a fallback
        if (playerManager.isHost) {
            const analytics = [
                {
                    analyticName: `levelDifficulties`,
                    uniqueKey: playerManager.getPlayerDetails(playerManager.getMyPlayerId()).profileId,
                },
    ...
    prepLevels(currentLevelIndex, startingLevel) {
    ...
    startGameplay() {
        ...
        if (playerManager.isHost) {
            console.log('GameManager startGameplay (Analytics): Registering level start.');
            const analytics = [
                {
    ...
    completeLevel(playerId) {
        ...
        console.log('GameManager completeLevel (Analytics): Registering level end.');
        const grade = this.currentElapsedTimeSec <= 5
            ? '<5'
    ...
    async onPlayerJoined({ playerId }) {
        ...
        console.log('GameManager onPLayerJoined (Analytics): Registering player join.');
        const analytics = [
            {
    ...
    onEvent_deferredAddPlayer(eventData) {
        const playerId = eventData?.playerId;
        if (this.collisionManager) {
            this.collisionManager.addPlayer(playerId);
        }
