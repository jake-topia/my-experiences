class GameManager extends SystemScript {
  ...
  safeZoneAreaWidth: number;
  safeZoneAreaHeight: number;
  currentLevelIndex: number;
  currentGameState: string; // 'STARTING', 'COUNTDOWN', 'ACTIVE', 'LEVEL_COMPLETE'
  gameStartTimeMs: number; // For calculating total level time
  currentElapsedTimeSec: number; // Stores the FINAL elapsed time for the level
  countdownSecondsDuration: number;
  pelletManager: PseudoAny;
  wallManager: PseudoAny;
  scoreManager: PseudoAny;
  timerManager: PseudoAny;
  countdownManager: PseudoAny;
  highScoreManager: PseudoAny;
  particleManager: PseudoAny;
  collisionManager: PseudoAny;
  utils: PseudoAny;
  naughtyList: PseudoAny;
  sprites: any[];
  reset: number;
  key: string;
  private playerJustTeleported: number;
  private playersCurrentlyTeleporting: PseudoAny; // Tracks who is teleporting

  /** Constructor: Initialize ALL properties here. */
  constructor() {
    this.key = stateManager.getVariable('PublicKey');
    ...
    console.log(
      `Safe Zone Info: Center X = ${this.safeZoneCenterX}, Center Y = ${this.safeZoneCenterY}, Width = ${this.safeZoneAreaWidth}, Height = ${this.safeZoneAreaHeight}`,
    );
  ...
  onInit() {
    if (playerManager.isHost) {
      console.log('GameManager onInit (Analytics): Registering host launch.');
      const analytics = [
        {
          analyticName: `gameLaunches`,
          profileId: playerManager.getPlayerDetails(
            playerManager.getPlayerIds()[0],
          ).profileId,
          uniqueKey: playerManager.getPlayerDetails(
            playerManager.getPlayerIds()[0],
          ).profileId,
        },
      ];
    ...
    try {
      ...
      // TEMP TESTING: Uncomment this line to wipe the WORLD data object on each host start.
      // if (playerManager.isHost) this.highScoreManager?.clearWorldDataObjectForTesting();
  ...
  onEvent_timerManagerReady() {
    if (this.currentGameState !== 'STARTING') {
      console.log(
        'GameManager: Received timerManagerReady event but game state is not STARTING. Ignoring.',
      );
      return;
    }
    console.log(
      'GameManager: Received timerManagerReady event. Starting game sequence...',
    );
  ...
  clearDifficultySprites() {
    ...
    console.log(
      'GameManager clearDifficultySprites (Analytics): Registering difficulty.',
    );
    const level = levelMap[this.currentLevelIndex] ?? 'DefaultLevel'; // The '??' provides a fallback
    if (playerManager.isHost) {
      const analytics = [
        {
          analyticName: `levelDifficulties`,
          uniqueKey: playerManager.getPlayerDetails(
            playerManager.getMyPlayerId(),
          ).profileId,
        },
        { analyticName: `difficultyOf-${level}s` },

        {
          analyticName: `numPlayersAtDifficultySelect-${playerManager.getPlayerIds.length}`,
        },
      ];
  ...
  prepLevels(currentLevelIndex: number, startingLevel: number) {
    this.clearDifficultySprites();
    this.currentLevelIndex = currentLevelIndex;
    this.wallManager.startingLevel = startingLevel;
  ...
  startNextLevel() {
    ...
    try {
      ...
      if (this.timerManager) {
        ...
        console.log(
          '!!! GameManager WARNING: Could not get TimerManager to clear timers.',
        );
      }
    } catch (e) {
    ...
    if (
      this.wallManager.totalLevels - 1 + this.wallManager.startingLevel >
    ...
    console.log(
      `GameManager: Preparing Level ${this.currentLevelIndex + 1}. State: ${this.currentGameState}`,
    );
    ...
    console.log(
      `GameManager: Starting ${initialCountdownValue} second countdown...`,
    );
    this.scoreManager?.displayMessage(
  ...
  scheduleNextCountdownTick(valueToDisplay: number) {
    if (this.currentGameState !== 'COUNTDOWN') return;
    ...
    try {
      if (this.timerManager) {
        ...
        console.log(
          '!!! GameManager ERROR: Could not get TimerManager to schedule countdown tick.',
        );
      }
    } catch (e) {
  ...
  onEvent_handleCountdownTick(payload: number | null) {
    // Expect number or null
    if (this.currentGameState !== 'COUNTDOWN') return;
    console.log('GameManager: Received handleCountdownTick event.');
    const currentValue = payload;
    console.log(
      `>>>   Received value: ${currentValue} (Type: ${typeof currentValue})`,
    );

    // *** Check for valid number (catches null, undefined, AND NaN) ***
    if (typeof currentValue !== 'number' || currentValue !== currentValue) {
      console.log(
        `!!! GameManager WARNING: Invalid value (type ${typeof currentValue} or NaN) received in handleCountdownTick. Stopping countdown.`,
      );
  ...
  onEvent_startGameplayNow() {
    if (this.currentGameState === 'COUNTDOWN') {
      ...
      console.log(
        `GameManager: Ignoring startGameplayNow event, state is ${this.currentGameState}.`,
      );
    }
  }
  ...
  startGameplay() {
    if (this.currentGameState === 'ACTIVE') {
      console.log(
        'GameManager: startGameplay called but already in ACTIVE state. Ignoring.',
      );
      return;
    }
    this.currentGameState = 'ACTIVE';
    console.log(
      `GameManager: Starting ACTIVE state for Level ${this.currentLevelIndex}`,
    );

    // --- Load Map ---
    console.log(
      `GameManager: Requesting WallManager load level ${this.currentLevelIndex}`,
    );
    try {
      this.wallManager?.clearLevel();
    ...
    if (playerManager.isHost) {
      console.log(
        'GameManager startGameplay (Analytics): Registering level start.',
      );
      const analytics = [
        {
          analyticName: `startLevels`,
          uniqueKey: playerManager.getPlayerDetails(
            playerManager.getMyPlayerId(),
          ).profileId,
        },
        { analyticName: `levelNums-${this.currentLevelIndex}` },

        {
          analyticName: `numPlayersAtLevelStart-${playerManager.getPlayerIds.length}`,
        },
      ];
  ...
  async completeLevel(playerId: number) {
    if (this.currentGameState !== 'ACTIVE') return;
    this.currentGameState = 'LEVEL_COMPLETE';
    console.log(
      `GameManager: Level ${this.currentLevelIndex} complete! Player: ${playerId}`,
    );

    this.stopGameplayTimer(); // Stop timer & calculate final time
    await this.recordScore(playerId, this.currentElapsedTimeSec);
    ...
    console.log(
      'GameManager completeLevel (Analytics): Registering level end.',
    );

    const grade =
      this.currentElapsedTimeSec <= 5
        ? '<5'
        : this.currentElapsedTimeSec >= 10
          ? '5-10'
          : this.currentElapsedTimeSec >= 20
            ? '10-20'
            : this.currentElapsedTimeSec >= 30
              ? '20-30'
              : '>30';

    const analytics = [
      {
        analyticName: `completedLevels`,
        profileId: playerManager.getPlayerDetails(playerId).profileId,
        uniqueKey: playerManager.getPlayerDetails(playerId).profileId,
      },
      { analyticName: `scoresOf-${grade}` },

      { analyticName: `levelNumCompleted-${this.currentLevelIndex}` },

      {
        analyticName: `numPlayersAtLevelComplete-${playerManager.getPlayerIds.length}`,
      },
    ];
      ...
      if (this.timerManager) {
        ...
        this.scoreManager.displayMessage(
          '\uD83D\uDE4CCongratulations ' +
            ...
            this.currentElapsedTimeSec,
          this.worldWidth / 2 - 200,
        );
      } else {
        console.log(
          '!!! GameManager ERROR: Could not get TimerManager to schedule next level.',
        );
      }
    } catch (e) {
      console.log(
        '!!! GameManager ERROR scheduling triggerNextLevel timer:',
        e,
      );
    }
  }
  ...
  onEvent_triggerNextLevel() {
    if (this.currentGameState === 'LEVEL_COMPLETE') {
      ...
      console.log(
        `GameManager: Ignoring triggerNextLevel event, state is ${this.currentGameState}.`,
      );
    }
  }
  ...
  stopGameplayTimer() {
    ...
    try {
      ...
      console.log(
        '!!! GameManager ERROR clearing gameplay update interval:',
        e,
      );
    }
  ...
  onEvent_updateGameplayDisplay() {
    ...
    const elapsedSeconds = Math.round(
      (Date.now() - this.gameStartTimeMs) / 1000.0,
    );
  ...
  async recordScore(playerId: number, score: number) {
    console.log(
      `GameManager: Telling ScoreManager to record score ${score} for Player ${playerId}`,
    );
    this.scoreManager?.recordFinalScore(playerId, score);
    await this.highScoreManager?.addScore(
      playerId,
      score,
      this.currentLevelIndex,
    );
  ...
  onEvent_playerHitWall(eventData: { playerId: number; wallId?: string }) {
    ...
    if (this.playerJustTeleported === playerId) {
      console.log(
        `GameManager: Ignoring playerHitWall for Player ${playerId} (just teleported).`,
      );
    ...
    console.log(
      `GameManager received event: playerHitWall for Player ${playerId}. Teleporting.`,
    );
    ...
    try {
      ...
      console.log(
        `GameManager: Scheduled resetLastTeleported timer for Player ${playerId}`,
      );
    } catch (e) {
      console.log(
        `!!! GameManager ERROR scheduling resetLastTeleported timer for Player ${playerId}:`,
        e,
      );
  ...
  onEvent_resetLastTeleported(payload: { playerId: number }) {
    const resetPlayerId = payload?.playerId;
    if (resetPlayerId === undefined) return;
    ...
    if (this.playerJustTeleported === resetPlayerId) {
      console.log(
        `GameManager: Resetting 'just teleported' flag for Player ${resetPlayerId}.`,
      );
      this.playerJustTeleported = null;
    } else {
      // Optional log: Another player was teleported more recently
      console.log(
        `GameManager: Ignoring reset timer for Player ${resetPlayerId}, another player ${this.playerJustTeleported}) was teleported more recently.`,
      );
    }
  }

  async onEvent_playerGotPellet(eventData: {
    playerId: number;
    pelletId?: string;
  }) {
    if (this.currentGameState !== 'ACTIVE') return;
    const playerId = eventData?.playerId;
    if (
      this.naughtyList[playerId] != undefined &&
      this.naughtyList[playerId].hitWall
    ) {
      return;
    }
    if (playerId !== undefined) {
      await this.completeLevel(playerId);
    }
  }
  async onPlayerJoined({ playerId }) {
    console.log("ON PLAYER JOINED BUG",this.collisionManager)
    if (this.collisionManager) {
      this.collisionManager.addPlayer(playerId);
    } else {
      console.log('GameManager onPlayerJoined: CollisionManager not ready, deferring addPlayer for ' + playerId);
      eventManager.emitWithTimeout('deferredAddPlayer', { playerId }, {timeoutId:"1234", timeoutSeconds:1});
    }

    console.log(
      'GameManager onPLayerJoined (Analytics): Registering player join.',
    );
    const analytics = [
      {
        analyticName: `playerJoins`,
        profileId: playerManager.getPlayerDetails(playerId).profileId,
        uniqueKey: playerManager.getPlayerDetails(playerId).profileId,
      },

      { analyticName: `numPlayersOnJoin-${playerManager.getPlayerIds.length}` },
    ];
  ...
  onEvent_deferredAddPlayer(eventData: PseudoAny) {
    const playerId = eventData?.playerId;
    if (this.collisionManager) {
      this.collisionManager.addPlayer(playerId);
    } else {
      console.log('GameManager: CollisionManager still not ready, retrying for ' + playerId);
      eventManager.emitWithTimeout('deferredAddPlayer', { playerId }, {timeoutSeconds:1, timeoutId:"retry"});
    }
  }
  // --- Helper Functions ---
  teleportPlayerToSafeZone(playerId: number) {
    if (
      typeof playerManager === 'undefined' ||
      typeof playerManager.teleportPlayers !== 'function'
    ) {
      console.log(
        '!!! GameManager ERROR: playerManager or playerManager.teleportPlayers not available!',
      );
      return;
    }
  ...
  onSpriteClicked(params: { event: PseudoEvent; sprite: PseudoSprite }) {
    if (params.sprite.uniqueId != 'scoreTextSprite') return;
    if (this.reset < 10) {
      this.reset++;
    } else {
      this.reset = 0;
  ...
  teleportAllPlayersToSafeZone() {
    console.log(
      'GameManager: Attempting to teleport all players to safe zone...',
    );
    ...
    if (
      ...
      console.log(
        '!!! GameManager ERROR: playerManager.getPlayerIds or playerManager.teleportPlayers not available!',
      );
      return;
    }
