class HighScoreManager extends SystemScript {
  // Declare properties
  highScoreText: PseudoSprite | null = null;
  worldWidth: number;
  worldHeight: number;
  // Use two parallel arrays, indexed by level number.
  scores: PseudoList; // Conceptually number[]
  players: PseudoList; // Conceptually string[]
  ...
  async onInit() {
    ...
    // if (playerManager.isHost) await this.clearWorldDataObjectForTesting(); // this.loadHighScores()
    ...
    try {
      ...
      if (sprite) {
        ...
        console.log(
          '!!! HighScoreManager FAILURE: Failed to create score text sprite!',
        );
        await this.loadHighScores(); // Still load scores
      }
    } catch (e) {
      console.log(
        '!!! HighScoreManager CRITICAL ERROR during sprite creation:',
        e,
      );
    }
  ...
  toggleHighScoreDisplay(active: boolean) {
    if (active) {
      this.updateDisplay();
    } else {
      this.displayMessage('');
  ...
  private updateDisplay() {
    ...
    console.log(
      'HighScoreManager.updateDisplay: Checking levels up to index ' +
        (maxIndex - 1),
    );
    ...
    for (
      let level = this.wallManager.startingLevel;
      level < maxIndex;
      level++
    ) {
      ...
      if (scoreValue !== undefined && playerName !== undefined) {
        if (typeof scoreValue === 'number' && typeof playerName === 'string') {
          console.log(
            'HighScoreManager.updateDisplay: Found data for level ' +
              level +
              ': Score=' +
              scoreValue +
              ', Player=' + // Changed from PlayerId
              playerName, // Use playerName directly
          );
          displayText +=
            'Level ' +
          ...
          console.log(
            '!!! HighScoreManager WARNING: Invalid data types found for level ' +
              level +
              ' (Score type: ' +
              typeof scoreValue +
    ...
    if (!foundScores) {
      console.log(
        'HighScoreManager.updateDisplay: No valid scores found to display.',
      );
  ...
  private async loadHighScores() {
    console.log(
      'HighScoreManager: Attempting to load high scores from cloud...',
    );
    const interactivePublicKey = stateManager.getVariable('PublicKey');

    if (!interactivePublicKey) {
      console.log(
        '!!! HighScoreManager CRITICAL ERROR: Cannot load scores, PublicKey is missing from stateManager.',
      );
      this.updateDisplay();
      return;
    }
    ...
    try {
      ...
      console.log(
        'HighScoreManager: Retrieved dataObject for loading:',
        dataObject,
      );
      ...
      if (
        ...
        console.log(
          'HighScoreManager: No existing high scores found in dataObject or structure is invalid.',
        );
      ...
      const loadedScores: PseudoList = [];
      const loadedPlayers: PseudoList = []; // Will store usernames

      console.log(
        'HighScoreManager: Processing loaded highScoresData:',
        highScoresData,
      );
      ...
      for (const levelKey in highScoresData) {
        if (levelKey.indexOf('level_') !== -1) {
          ...
          if (levelNumber < 0) {
            console.log(
              `HighScoreManager: Invalid level number parsed from key '${levelKey}'. Skipping.`,
            );
            continue;
          }
          ...
          if (
            ...
            console.log(
              `HighScoreManager: Invalid score data structure or types for key '${levelKey}':`,
              scoreData,
              '. Skipping.',
            );
            continue;
          ...
          console.log(
            `HighScoreManager: Loading data for level index ${levelNumber}: Player='${scoreData.playerName}', Score=${scoreData.score}`,
          );
          ...
          console.log(
            `HighScoreManager: Skipping key '${levelKey}' (not own property or doesn't match format).`, // Kept this log
          );
        }
      }
      ...
      console.log(
        '!!! HighScoreManager ERROR during loadHighScores operation:',
        error,
      );
      ...
      console.log(
        'HighScoreManager: Calling updateDisplay after load attempt.',
      );
  ...
  async postHighScore(
    ...
    console.log(
      `HighScoreManager: Attempting to post high score for level ${level} - Player: '${playerName}', Score: ${score}`,
    );
    const interactivePublicKey = stateManager.getVariable('PublicKey');

    if (!interactivePublicKey) {
      console.log(
        '!!! HighScoreManager CRITICAL ERROR: Cannot post score, PublicKey is missing from stateManager.',
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        'HighScoreManager: Retrieved existing dataObject:',
        dataObject,
      );
      ...
      if (!dataObject) {
        console.log(
          'HighScoreManager: No existing dataObject found. Creating new structure.',
        );
        dataObject = {
        ...
        if (
          ...
          console.log(
            "HighScoreManager: dataObject found, but 'highScores' property missing or invalid. Initializing.",
          );
          dataObject.highScores = {};
        }
        console.log(
          `HighScoreManager: Updating high score for key ${levelKey}`,
        );
      ...
      console.log(
        ...
        'HighScoreManager: Saving updated dataObject:',
        JSON.stringify(dataObject),
      );
      console.log(
        'HighScoreManager: Triggering GAME_HIGH_SCORE for ' + activityReason,
      );
      await integrationsManager.setWorldActivity({
      ...
      console.log(
        '!!! HighScoreManager ERROR during postHighScore operation:',
        error,
      );
    }
  }
  ...
  displayMessage(message: string) {
    ...
    console.log(
      'HighScoreManager: Displaying message on sprite - "' + message + '"',
    );
    try {
      ...
      console.log(
        '!!! HighScoreManager ERROR updating sprite text (message):',
        e,
      );
    }
  }
   ...
   */
  async addScore(playerId: number, score: number, level: number) {
    //level -= 1;
    const playerCheck = playerManager.getPlayerIds();
    //if (!playerCheck.includes(playerId)) return;
    console.log(
      'HighScoreManager.addScore received: player=' +
    ...
    if (tempArray[level] !== undefined) currentScore = tempArray[level] ?? 0;
    console.log(
      'HighScoreManager.addScore: Current score for level ' +
        level +
        ' is: ' +
        currentScore +
        ' (type: ' +
    ...
    let activityReason = 'didNotQualify';
    if (tempArray[level] === undefined) {
      console.log(
        'HighScoreManager.addScore: No existing score defined for level ' +
          level +
          '. Setting new score.',
      );
      shouldUpdate = true;
      activityReason = 'firstRecordedScore';
    } else if (
      (typeof currentScore === 'number' && score < currentScore) ||
      currentScore == 0
    ) {
      console.log(
      ...
      shouldUpdate = true;
      activityReason = 'beatExistingScore';
    } else {
      console.log(
        'HighScoreManager.addScore: New score (' +
          score +
    ...
    if (shouldUpdate) {
      ...
      console.log(
        `HighScoreManager.addScore: Using player name: '${playerName}'`,
      );
      // --- End Get Player Name ---
      ...
      try {
        if (this.scores.length > level) {
          console.log(
            'updating without push... scores.length = ' + this.scores.length,
          );
        ...
        console.log(
          'HighScoreManager.addScore: Updated arrays. scores[' +
            level +
            ']=' +
            this.scores[level] +
            ', players[' +
            level +
            ']=' +
            this.players[level], // Log the name stored
        );

        // --- Post to Cloud (using playerName) ---
        await this.postHighScore(playerName, score, level, activityReason); // Pass username string
        // --- End Post to Cloud ---

        this.updateDisplay(); // Call updateDisplay AFTER attempting the update
      } catch (e) {
        console.log(
          '!!! HighScoreManager CRITICAL ERROR during array assignment for level ' +
            level +
            ': ',
          e,
        );
  ...
  async triggerHighScoreActivity(activityReason: string) {
    try {
      const interactivePublicKey = stateManager.getVariable('PublicKey');
      if (!interactivePublicKey) return;
      console.log(
        'HighScoreManager: setWorldActivity GAME_HIGH_SCORE reason=' +
          activityReason,
      );
      await integrationsManager.setWorldActivity({
        type: 'GAME_HIGH_SCORE',
  ...
  async clearWorldDataObjectForTesting() {
    const interactivePublicKey = stateManager.getVariable('PublicKey');
    if (!interactivePublicKey) {
      console.log(
        '!!! HighScoreManager CRITICAL ERROR: Cannot clear world data object, PublicKey is missing from stateManager.',
      );
      return;
    }

    console.log(
      'HighScoreManager TESTING: Clearing WORLD data object on host start.',
    );
    await integrationsManager.updateDataObject({
      interactivePublicKey: interactivePublicKey,
      scope: 'WORLD',
  ...
  async populateTempHighScores() {
    console.log(
      'HighScoreManager: --- RUNNING TEMPORARY HIGH SCORE POPULATION (Levels 0-9) ---',
    ); // Warn that it's temporary
    const interactivePublicKey = stateManager.getVariable('PublicKey');

    if (!interactivePublicKey) {
      console.log(
        '!!! HighScoreManager CRITICAL ERROR: Cannot populate temp scores, PublicKey is missing from stateManager.',
      );
    ...
    try {
      ...
      console.log(
        'HighScoreManager (Temp Populate): Retrieved existing dataObject:',
        dataObject,
      );
      ...
      if (!dataObject || typeof dataObject !== 'object') {
        console.log(
          'HighScoreManager (Temp Populate): No existing dataObject found. Creating new one.',
        );
        ...
        console.log(
          "HighScoreManager (Temp Populate): Existing dataObject missing 'highScores'. Initializing 'highScores'.",
        );
        dataObject.highScores = {}; // Add the highScores property if missing
      } else {
        console.log(
          'HighScoreManager (Temp Populate): Existing dataObject and highScores found. Proceeding.',
        );
      }
      ...
      console.log(
        `HighScoreManager (Temp Populate): Setting scores for levels 0-9 to ${tempScore} by '${tempPlayerName}'...`,
      );
      ...
      console.log(
        'HighScoreManager (Temp Populate): Finished setting temp data in object.',
      );

      // 5. Save the modified data object back to the cloud
      console.log(
        'HighScoreManager (Temp Populate): Saving updated dataObject with temp scores:',
        JSON.stringify(dataObject),
      );
      await integrationsManager.updateDataObject({
        interactivePublicKey: interactivePublicKey,
      ...
      console.log(
        'HighScoreManager: --- Successfully populated cloud with temp high scores (Levels 0-9) ---',
      ); // Warn on success too
    } catch (error) {
      console.log(
        '!!! HighScoreManager ERROR during populateTempHighScores operation:',
        error,
      );
    }
  }
