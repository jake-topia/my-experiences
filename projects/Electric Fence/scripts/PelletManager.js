...
class PelletManager extends SystemScript {
    ...
    spawnPellet(x, y) {
        // --- Log Entry Point ---
        console.log(`PelletManager: ----- ENTERED spawnPellet(x=${x}, y=${y}) -----`);
        // --- End Log ---
        ...
        if (spawnX !== x || spawnY !== y) {
            console.log(`PelletManager WARNING: Invalid coordinates provided. Spawning at fallback (${spawnX}, ${spawnY})`);
        }
        console.log(`PelletManager: Preparing to spawn '${this.pelletAssetKey}' at (${spawnX}, ${spawnY}).`);
        try {
            ...
            if (pelletSprite) {
                console.log('PelletManager: addSprite successful. Attaching component...');
            ...
            console.log(`!!! PelletManager CRITICAL ERROR during pellet creation:`, error);
            this.activePelletSprite = null;
        }
    ...
    onEvent_requestSpawnPellet(eventData) {
        console.log(`PelletManager received event: requestSpawnPellet for level ${eventData?.levelIndex}`, eventData);
        if (eventData &&
            ...
            console.log('!!! PelletManager ERROR: Invalid or missing coordinates in requestSpawnPellet event. Spawning at default.');
