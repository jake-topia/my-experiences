...
class PongAnalyticsManager extends SystemScript {
    ...
    currentGamePaddleHits; // Track paddle hits in current game
    currentGameMultiballs; // Track multiball triggers in current game
    onInit() {
    ...
    trackGameStart(starterPlayerId, playerCount) {
        ...
        try {
            ...
            console.log('[PongAnalytics] Game start tracked:', playerCount, 'players');
        }
        catch (e) {
     ...
     */
    trackRoundCompletion(roundNumber, winnerTeam, playerCount, gameManager) {
        if (!this.analyticsPublicKey)
            return;
        try {
            const analytics = [];
            ...
            if (winnerTeam) {
                ...
                if (winnerPlayerId) {
                    const winnerProfile = playerManager.getPlayerDetails(winnerPlayerId)?.profileId;
                    if (winnerProfile) {
                        analytics.push({
                        ...
                        analytics.push({
                            analyticName: 'tpongRound' + roundNumber + 'Wins-' + playerCount + 'p',
                            profileId: winnerProfile,
                        });
                    }
                }
                ...
                for (let i = 0; i < activeTeams.length; i++) {
                    ...
                    if (team !== winnerTeam) {
                        ...
                        if (loserPlayerId) {
                            const loserProfile = playerManager.getPlayerDetails(loserPlayerId)?.profileId;
                            if (loserProfile) {
                                analytics.push({
                                ...
                                analytics.push({
                                    analyticName: 'tpongRound' + roundNumber + 'Losses-' + playerCount + 'p',
                                    profileId: loserProfile,
                                });
                            }
                        }
                ...
                for (let i = 0; i < activeTeams.length; i++) {
                    ...
                    if (playerId) {
                        ...
                        if (profile) {
                            ...
                            analytics.push({
                                analyticName: 'tpongRound' + roundNumber + 'Ties-' + playerCount + 'p',
                                profileId: profile,
                            });
                        }
                    }
     ...
     * @param gameManager - Reference to game manager for accessing team data
     */
    trackTournamentCompletion(winnerTeam, playerCount, gameManager) {
        if (!this.analyticsPublicKey)
            return;
        try {
            const analytics = [
            ...
            if (winners.length === 0) {
                ...
                if (winnerId) {
                    const winnerProfile = playerManager.getPlayerDetails(winnerId)?.profileId;
                    if (winnerProfile) {
                        analytics.push({
                ...
                for (let i = 0; i < activeTeams.length; i++) {
                    ...
                    if (team !== winners[0]) {
                        ...
                        if (loserId) {
                            const loserProfile = playerManager.getPlayerDetails(loserId)?.profileId;
                            if (loserProfile) {
                                analytics.push({
     ...
     */
    getTeamPlayerId(teamId, gameManager) {
        if (!gameManager)
            return null;
        let teamList = null;
        if (teamId === 'red')
            teamList = gameManager.teamRed;
        else if (teamId === 'blue')
            teamList = gameManager.teamBlue;
        else if (teamId === 'green')
            teamList = gameManager.teamGreen;
        else if (teamId === 'yellow')
            teamList = gameManager.teamYellow;
     ...
     */
    trackMultiballTrigger(ballNumber, playerCount) {
        ...
        try {
            ...
            console.log('[PongAnalytics] Multiball trigger tracked: ball #' + ballNumber);
        }
        catch (e) {
