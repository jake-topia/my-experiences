...
class PongBallManager extends SystemScript {
    ...
    balls; // Array of ball objects: { id, x, y, velX, velY, speed, isActive }
    goalJustScored; // Prevents multiple goal triggers
    pendingGoals; // Goals to process after physics step
    // --- Player Count (for goal detection) ---
    playerCount;
    originalPlayerCount; // Original layout (2 or 4) - determines which edges are goal zones
    eliminatedTeams; // Teams eliminated in sudden death (walls, not goals)
    debugLogging;
    lastTouchPlayerId;
    lastTouchTeam;
    resetTimeoutId;
    // --- Fair Ball Launch Queue ---
    targetQueue; // Randomized queue of teams for fair ball targeting
    // --- Ball Emoji System ---
    teamEmojis; // Team -> emoji arrays
    funnyEmojis; // Fun emoji variations to occasionally use
    ballHitCount; // Track hits to trigger funny emojis
    onInit() {
    ...
    isTeamEliminated(teamId) {
        for (let i = 0; i < this.eliminatedTeams.length; i++) {
            if (this.eliminatedTeams[i] === teamId)
                return true;
        }
        return false;
    }
    ...
    refillAndShuffleTargetQueue() {
        ...
        console.log('[PongBM] Refilled target queue: ' + this.targetQueue.join(','));
    }
    /**
    ...
    popNextTarget() {
        ...
        this.targetQueue = newQueue;
        console.log('[PongBM] Ball target: ' +
            target +
            ' (remaining in queue: ' +
    ...
    resetBall(_targetTeam) {
        ...
        this.logDebug('[PongBM] Ball velocity: (' + ballVelX + ', ' + ballVelY + ')');
    ...
    spawnAdditionalBall() {
        ...
        console.log('[PongBM] Spawned ball #' +
            ballNumber +
            ' (' +
            newBallId +
            ') at (' +
    ...
    onSpriteCollisionStart({ sprite1, sprite2, }) {
        ...
        this.logDebug('[PongBM] Ball collided with paddle: ' +
            otherId +
            ' at (' +
            paddleX +
            ', ' +
     ...
     */
    setPlayerCount(count, isOriginal) {
        this.playerCount = count;
        if (isOriginal) {
            this.originalPlayerCount = count;
            console.log('[PongBM] Set original player count to ' +
                count +
     ...
     */
    handleGoal(scoredAgainstTeam, ballId) {
        console.log('[PongBM] Goal scored against: ' +
            scoredAgainstTeam +
            ' by ball: ' +
            ballId);
     ...
     */
    handlePaddleCollision(ball, collision) {
        ...
        ball.speed = Math.min(ball.speed + this.config.ballSpeedIncrement, this.config.ballSpeedMax);
    ...
    extractPlayerIdFromPaddleId(paddleId) {
        const prefix = 'paddle_';
        if (paddleId.indexOf(prefix) !== 0)
            return -1;
    ...
    resolveScoringTeam(scoredAgainstTeam) {
        if (this.lastTouchTeam)
        ...
        for (let i = 0; i < teamOrder.length; i++) {
            ...
            if (candidate === scoredAgainstTeam)
                continue;
            if (this.getTeamSize(candidate) > 0)
                return candidate;
        }
        return '';
    }
    ...
    changeRandomBallColor(ballId) {
        ...
        if (this.ballHitCount % 10 === 0) {
            ...
            // Default white
            ballEmoji =
                this.teamEmojis.white[Math.floor(Math.random() * this.teamEmojis.white.length)];
        }
        // Update ball emoji
