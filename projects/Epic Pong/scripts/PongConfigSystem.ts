class PongConfigSystem extends SystemScript {
  ...
  teamColors: any; // { red, blue, green, yellow }
  teamNames: any; // { red, blue, green, yellow }
  ...
  countdownDuration: number; // seconds for pre-game countdown
  winningScore: number; // DEPRECATED - replaced by lives system
  gameOverDisplayDuration: number; // seconds to show game over screen

  // --- Lives System (NEW) ---
  startingLives: number; // lives each player starts with

  // --- Round System (NEW) ---
  currentRound: number; // 1, 2, or 3
  roundMode: string; // 'button' | 'coordinate' | 'bodypaddle'
  roundDelayMs: number; // delay between rounds (milliseconds)
  roundCelebrationMs: number; // celebration time for round winner (milliseconds)
  roundCountdownMs: number; // countdown with instructions (milliseconds)
  tournamentCelebrationMs: number; // celebration time after tournament (milliseconds)
  eliminationPopupMs: number; // how long to show elimination popup in sudden death

  // --- Paddle Configuration ---
  paddleWidth: number;
  paddleHeight: number;
  paddleSpeed: number; // pixels per frame
  paddleOffset: number; // distance from edge of play area

  // --- Ball Configuration ---
  ballSize: number;
  ballSpeedInitial: number;
  ballSpeedIncrement: number; // speed increase per paddle hit
  ballSpeedMax: number;
  multiballHitThreshold: number; // consecutive hits to spawn second ball

  // --- Control Configuration ---
  buttonWidth: number;
  buttonHeight: number;
  buttonGap: number; // spacing between buttons
  ...
  laneWidth: number; // Width of control lane around play area
  playAreaX: number; // X position of central play area
  playAreaY: number; // Y position of central play area
  playAreaWidth: number; // Width of central play area
  playAreaHeight: number; // Height of central play area

  // --- Camping Detection (Button Mode Only) ---
  campingTimeoutMs: number; // Time before teleporting player out of play area (milliseconds)

  // --- Body Paddle Mode (DEPRECATED - now part of round system) ---
  bodyPaddleMode: boolean; // DEPRECATED - use roundMode instead
  bodyPaddleOffsetX: number; // Horizontal offset for paddle from player center
  bodyPaddleOffsetY: number; // Vertical offset for paddle from player center

  // --- UI Text Strings (for easy copy editing) ---
  uiText: any; // All on-screen text messages
   ...
   */
  getTeamColor(teamId: string): number {
  ...
  getTeamName(teamId: string): string {
    if (teamId === 'red') return this.teamNames.red;
    if (teamId === 'blue') return this.teamNames.blue;
    if (teamId === 'green') return this.teamNames.green;
    if (teamId === 'yellow') return this.teamNames.yellow;
   ...
   */
  getPlayerNameForTeam(teamId: string, gameManager: any): string {
    if (!gameManager) return this.getTeamName(teamId);
    ...
    if (!playerId) return this.getTeamName(teamId);
   ...
   */
  getTeamEmoji(teamId: string): string {
    const emojis: any = {
      red: '🔴',
      blue: '🔵',
      green: '🟢',
   ...
   */
  getAvailableTeams(playerCount: number): PseudoList {
    const teams: any = ['red', 'blue'];
    if (playerCount >= 3) teams.push('green');
    if (playerCount >= 4) teams.push('yellow');
    return teams as any;
  }
   ...
   */
  getPaddlePosition(teamId: string, playerCount: number): any {
   ...
   */
  getGoalArea(teamId: string, playerCount: number): any {
