...
class PongGameManager extends SystemScript {
  ...
  // --- Game State ---
  currentState: string; // LOBBY | COUNTDOWN | ACTIVE | GAME_OVER

  // --- Player Management ---
  teamRed: PseudoList; // Player IDs on red team
  teamBlue: PseudoList; // Player IDs on blue team
  teamGreen: PseudoList; // Player IDs on green team
  teamYellow: PseudoList; // Player IDs on yellow team
  scoreRed: number; // DEPRECATED - replaced by lives
  scoreBlue: number; // DEPRECATED - replaced by lives
  scoreGreen: number; // DEPRECATED - replaced by lives
  scoreYellow: number; // DEPRECATED - replaced by lives
  activePlayers: PseudoList; // list of player IDs in current game
  playerColors: any; // team -> player tint color hex
  ...
  nextRoundNumber: number; // Next round to start (for countdown display)
  roundWins: PseudoMap; // playerId (red/blue/green/yellow) -> wins count
  tournamentInProgress: boolean;
  roundTransitionState: string; // '' | 'celebration' | 'countdown'
  roundTransitionEndTime: number; // timestamp when current transition ends
  roundCountdownDisplay: number; // last displayed countdown number
  originalPlayerCount: number; // Original game layout (2 or 4) - determines paddle/goal positions

  // --- Sudden Death System (NEW) ---
  suddenDeathActive: boolean; // true when in sudden death mode
  eliminatedTeams: PseudoList; // teams eliminated in sudden death
  activeSuddenDeathTeams: PseudoList; // teams still alive in sudden death

  // --- Player Leave Tracking ---
  leftTeams: PseudoList; // teams whose players left mid-tournament (persists across rounds)

  // --- Subsystems ---
  paddleManager: any; // PongPaddleManager
  ballManager: any; // PongBallManager
  uiManager: any; // PongUIManager
  analyticsManager: any; // PongAnalyticsManager

  // --- Countdown State (CTF pattern - no timers, use Date.now()) ---
  countdownEndTime: number; // timestamp when countdown ends
  lastCountdownDisplay: number; // last displayed countdown number

  // --- Game Over State ---
  winningTeam: string;
  gameOverTimeoutId: any;
  scoreTimeoutId: any;
  debugLogging: boolean;

  // --- Multi-ball Feature ---
  consecutiveHits: number; // Count hits without scoring

  // --- Camping Detection (Button Mode Only) ---
  campingTimers: PseudoMap; // playerId -> timestamp when they entered play area
  ...
  onInit() {
    ...
    this.teamRed = [] as any;
    this.teamBlue = [] as any;
    this.teamGreen = [] as any;
    this.teamYellow = [] as any;
    this.logDebug(
      '[PongGM] DEBUG: Team lists initialized - Red:',
      typeof this.teamRed,
      'isArray:',
      Array.isArray(this.teamRed),
    );
    this.logDebug(
      '[PongGM] DEBUG: Team lists initialized - Blue:',
      typeof this.teamBlue,
      'isArray:',
      Array.isArray(this.teamBlue),
    );
    this.scoreRed = 0;
    this.scoreBlue = 0;
    this.scoreGreen = 0;
    this.scoreYellow = 0;
    this.activePlayers = [] as any;
    ...
    this.campingTimers = {} as PseudoMap; // Initialize camping detection
    ...
    this.roundWins = {} as PseudoMap;
    this.roundWins.red = 0 as any;
    this.roundWins.blue = 0 as any;
    this.roundWins.green = 0 as any;
    this.roundWins.yellow = 0 as any;
    ...
    this.eliminatedTeams = [] as any;
    this.activeSuddenDeathTeams = [] as any;

    // Initialize player leave tracking
    this.leftTeams = [] as any;
  ...
  clearScoreResetTimer() {
    if (!this.scoreTimeoutId) return;

    const timer =
      typeof timerManager !== 'undefined' && timerManager
        ? (timerManager as any)
        : null;
    ...
    try {
      ...
      console.warn(
        '[PongGM] Failed to clear timer "' + this.scoreTimeoutId + '"',
        err,
      );
    }
  ...
  showLobby() {
    ...
    this.eliminatedTeams = [] as any;
    this.activeSuddenDeathTeams = [] as any;
    this.leftTeams = [] as any; // Reset left teams tracking for new game
   ...
   */
  joinTeam(playerId: number, teamId: string): boolean {
    if (!playerManager.isHost) return false;

    this.logDebug(
      '[PongGM] joinTeam called - playerId:',
      playerId,
      'teamId:',
      teamId,
    );
    this.logDebug(
      '[PongGM] DEBUG: Before join - teamRed type:',
      typeof this.teamRed,
      'isArray:',
      Array.isArray(this.teamRed),
      'length:',
    ...
    this.logDebug(
      '[PongGM] DEBUG: After push - lengths: R=' +
        this.teamRed.length +
        ' B=' +
  ...
  getPlayerTeam(playerId: number): string | null {
    for (let i = 0; i < this.teamRed.length; i++) {
      if (this.teamRed[i] === playerId) return 'red';
    }
    for (let i = 0; i < this.teamBlue.length; i++) {
      if (this.teamBlue[i] === playerId) return 'blue';
    }
    for (let i = 0; i < this.teamGreen.length; i++) {
      if (this.teamGreen[i] === playerId) return 'green';
    }
    for (let i = 0; i < this.teamYellow.length; i++) {
      if (this.teamYellow[i] === playerId) return 'yellow';
    }
    return null;
  }
   ...
   * 0 lives = (empty)
   */
  getLivesAsHearts(teamId: string): string {
    let lives = 0;
    if (teamId === 'red') lives = this.livesRed;
    else if (teamId === 'blue') lives = this.livesBlue;
    else if (teamId === 'green') lives = this.livesGreen;
    else if (teamId === 'yellow') lives = this.livesYellow;
   ...
   * etc.
   */
  getWinsAsCrowns(teamId: string): string {
   ...
   */
  updatePlayerNameplate(playerId: number) {
    ...
    let nameplate = '';
    if (isCamping) nameplate += '⛺ ';
    if (crowns) nameplate += crowns + ' ';
    if (hearts) nameplate += hearts + ' ';
    nameplate += username;
   ...
   */
  autoAssignTeams(playerCount: number) {
  ...
  startGame() {
    ...
    if (playerCount !== 2 && playerCount !== 4) {
      console.warn(
        '[PongGM] Need exactly 2 or 4 players to start (currently have ' +
          playerCount +
          ')',
      );
    ...
    console.log(
      '[PongGM] Original player count set to ' +
        teamCount +
        ' (determines layout)',
    );
    ...
    if (this.currentState === 'COUNTDOWN') {
      ...
      if (remaining !== this.lastCountdownDisplay && remaining > 0) {
        ...
        if (this.uiManager) {
          ...
          console.error(
            '[PongGM] ERROR: UI manager unavailable for GO! display',
          );
        }
      }
    }
  ...
  beginActiveGame() {
    ...
    console.log(
      '[PongGM] ⚠️ DEBUG: roundMode at beginActiveGame =',
      this.config.roundMode,
    );
    console.log('[PongGM] ⚠️ DEBUG: currentRound =', this.currentRound);
    console.log(
      '[PongGM] ⚠️ DEBUG: originalPlayerCount =',
      this.originalPlayerCount,
    );
    console.log(
      '[PongGM] ⚠️ DEBUG: activePlayers.length =',
    ...
    if (this.paddleManager) {
      for (let i = 0; i < this.activePlayers.length; i++) {
        ...
        if (teamId) {
          ...
          const paddlePos = this.config.getPaddlePosition(
            teamId,
            layoutPlayerCount,
          );
  ...
  onPaddleHit() {
    ...
    console.log(
      '[PongGM] Paddle hit! Consecutive hits: ' + this.consecutiveHits,
    );
    ...
    if (
      ...
      var ballNumber =
        Math.floor(this.consecutiveHits / this.config.multiballHitThreshold) +
        1;
      console.log(
        '[PongGM] Multiball threshold reached! Spawning ball #' +
          ballNumber +
          ' at ' +
          this.consecutiveHits +
          ' hits',
      ...
      if (this.analyticsManager) {
        this.analyticsManager.trackMultiballTrigger(
          ballNumber,
   ...
   */
  onScore(zoneTeam: string, lastTouchTeam?: string) {
    if (!playerManager.isHost) return;
    ...
    if (zoneTeam === 'red') this.livesRed--;
    else if (zoneTeam === 'blue') this.livesBlue--;
    else if (zoneTeam === 'green') this.livesGreen--;
    else if (zoneTeam === 'yellow') this.livesYellow--;

    // Also update old score system for compatibility (deprecated)
    if (zoneTeam === 'red') this.scoreRed++;
    else if (zoneTeam === 'blue') this.scoreBlue++;
    else if (zoneTeam === 'green') this.scoreGreen++;
    else if (zoneTeam === 'yellow') this.scoreYellow++;
  ...
  checkGameOver() {
    ...
    if (this.suddenDeathActive) {
      ...
      let eliminatedTeam: string | null = null;
      ...
      if (eliminatedTeam) {
        // Player eliminated!
        console.log(
          '[PongGM] SUDDEN DEATH - ' + eliminatedTeam + ' ELIMINATED!',
        );

        // Remove from active list (PseudoList-compatible - rebuild without eliminated team)
        const newActiveTeams: PseudoList = [] as any;
        ...
        console.log(
          '[PongGM] After elimination - activeSuddenDeathTeams.length = ' +
        ...
        if (this.activeSuddenDeathTeams.length === 1) {
          ...
          const winnerText = this.config.uiText.suddenDeathWinner.replace(
            '{player}',
            winnerName,
          );
          if (this.uiManager) {
          ...
          if (winnerPlayerId) {
            ...
            if (details) {
              ...
              try {
                ...
                console.warn(
                  '[PongGM] Failed to trigger sudden death winner particle',
                  e,
                );
              }
            }
          ...
          console.log(
            '[PongGM] SUDDEN DEATH TIE - All players eliminated simultaneously',
          );
          this.currentState = 'GAME_OVER';
          this.suddenDeathActive = false;
          this.endTournament();
          ...
          const playerName = this.config.getPlayerNameForTeam(
            eliminatedTeam,
            this,
          );
          if (this.uiManager) {
          ...
          console.log(
            '[PongGM] Creating elimination hide timer for ' + durationMs + 'ms',
          );
          timerManager.createTimer({
            duration: durationMs,
            onComplete: () => {
          ...
          const randomIdx = Math.floor(
            Math.random() * this.activeSuddenDeathTeams.length,
          );
          const targetTeam = this.activeSuddenDeathTeams[randomIdx];
          console.log(
            '[PongGM] SUDDEN DEATH continues with ' +
              this.activeSuddenDeathTeams.length +
              ' players',
          );
      ...
      const targetTeam = this.activeSuddenDeathTeams[randomIdx];
      console.log(
        '[PongGM] SUDDEN DEATH - score detected but no elimination (possible wall bounce issue)',
      );
      this.resetBallAfterScore(targetTeam);
      return;
    }
    ...
    const hasTeamLeft = (teamId: string): boolean => {
      for (let k = 0; k < this.leftTeams.length; k++) {
        if (this.leftTeams[k] === teamId) return true;
      }
      return false;
    };
    ...
    if (!hasLoser) {
      ...
      if (this.teamRed.length > 0 && this.livesRed > 0) activeTeams.push('red');
    ...
    if (this.teamRed.length > 0) activeTeams.push('red');
    if (this.teamBlue.length > 0) activeTeams.push('blue');
    if (this.teamGreen.length > 0) activeTeams.push('green');
    if (this.teamYellow.length > 0) activeTeams.push('yellow');
    ...
    if (this.tournamentInProgress) {
      ...
      if (
        ...
        for (let m = 0; m < winners.length; m++) {
          ...
          this.roundWins[survivorTeam] = (currentWins + 1) as any;
        }
        ...
        console.log(
          '[PongGM] Round ' + this.currentRound + ' won by ' + winningTeam,
        );
   ...
   */
  resetBallAfterScore(targetTeam: string) {
    this.clearScoreResetTimer();
  ...
  onEvent_tpongBallResetNow(payload: any) {
    if (!playerManager.isHost) return;
   ...
   */
  endRound(winningTeam: string | null) {
    if (!playerManager.isHost) return;
    ...
    if (winningTeam && winningTeam.indexOf('survivors:') !== 0) {
      ...
      this.roundWins[winningTeam] = (currentWins + 1) as any;
      console.log(
        '[PongGM] ' +
          winningTeam +
          ' wins Round ' +
      ...
      console.log(
        '[PongGM] Round ' + this.currentRound + ' - survivor round complete',
      );
    }
    ...
    if (this.analyticsManager) {
      ...
      if (winningTeam && winningTeam.indexOf('survivors:') === 0) {
        ...
        for (let i = 0; i < survivorTeams.length; i++) {
          this.analyticsManager.trackRoundCompletion(
            this.currentRound,
            survivorTeams[i],
            this.getAssignedTeamCount(),
            this,
          );
        }
      } else {
    ...
    if (this.uiManager) {
      ...
      if (winningTeam && winningTeam.indexOf('survivors:') === 0) {
        ...
        for (let i = 0; i < survivorTeams.length; i++) {
          survivorNames.push(
            this.config.getPlayerNameForTeam(survivorTeams[i], this),
          );
        }
        ...
        roundWinnerText = this.config.uiText.roundWinner.replace(
          '{player}',
          playerName,
        );
      } else {
   ...
   */
  onEvent_tpongNextRound(payload: any) {
    if (!playerManager.isHost) return;
    const nextRound =
      payload && payload.nextRound ? payload.nextRound : this.currentRound + 1;
   ...
   */
  startNextRound(roundNumber: number) {
    if (!playerManager.isHost) return;
    ...
    if (this.leftTeams.length > 0) {
      console.log(
        '[PongGM] Re-applying eliminated status for left teams: ' +
      ...
      for (let i = 0; i < this.leftTeams.length; i++) {
        ...
        if (leftTeam === 'red') this.livesRed = 0;
        else if (leftTeam === 'blue') this.livesBlue = 0;
        else if (leftTeam === 'green') this.livesGreen = 0;
        else if (leftTeam === 'yellow') this.livesYellow = 0;
  ...
  endTournament() {
    ...
    if (tournamentWinners.length > 1) {
      console.log(
        '[PongGM] Tournament TIE - Starting SUDDEN DEATH with: ' +
    ...
    const celebrationText = this.config.uiText.tournamentWinner.replace(
      '{player}',
      playerName,
    );
    ...
    if (this.analyticsManager) {
      ...
      this.analyticsManager.trackTournamentCompletion(
        finalWinner,
        this.getAssignedTeamCount(),
        this,
      );
    ...
    console.log(
      '[PongGM] Tournament complete - host will leave game in 5 seconds',
    );
    timerManager.createTimer({
      duration: 5000,
      onComplete: () => {
   ...
   */
  startSuddenDeath(tiedTeams: string[]) {
    if (!playerManager.isHost) return;
    ...
    for (let k = 0; k < tiedTeams.length; k++) {
      ...
      if (team === 'red') this.livesRed = 1;
      else if (team === 'blue') this.livesBlue = 1;
      else if (team === 'green') this.livesGreen = 1;
      else if (team === 'yellow') this.livesYellow = 1;
    }
   ...
   */
  convertGoalZoneToWall(teamId: string) {
    if (!playerManager.isHost) return;
   ...
   */
  getAssignedTeamCount(): number {
    let count = 0;
    if (this.teamRed.length > 0) count++;
    if (this.teamBlue.length > 0) count++;
    if (this.teamGreen.length > 0) count++;
    if (this.teamYellow.length > 0) count++;
    return count;
  }
   ...
   */
  getTeamPlayerId(teamId: string): number | null {
    if (teamId === 'red' && this.teamRed.length > 0) return this.teamRed[0];
    if (teamId === 'blue' && this.teamBlue.length > 0) return this.teamBlue[0];
  ...
  onPlayerJoined(o: { playerId: number }) {
    ...
    if (
      ...
      console.log(
        '[PongGM] Game in progress - player ' + playerId + ' will spectate',
      );
      ...
      const details = playerManager.getPlayerDetails(playerId);
      const username =
        details && details.username ? details.username : 'P' + playerId;
    ...
    if (this.suddenDeathActive) {
      console.log(
        '[PongGM] Sudden death active - player ' +
          playerId +
    ...
    if (this.currentState === 'LOBBY') {
      ...
      console.log(
        '[PongGM] Auto-assigned player ' + playerId + ' to ' + targetTeam,
      );
    }
  ...
  onPlayerLeft(o: { playerId: number }) {
    ...
    const removeFromList = (list: any, pid: number) => {
    ...
    if (
      ...
      if (wasActive && leavingTeam) {
        console.log(
          '[PongGM] Active player left from team ' +
            leavingTeam +
            ' - continuing game',
        );
        ...
        if (leavingTeam === 'red') this.livesRed = 0;
        else if (leavingTeam === 'blue') this.livesBlue = 0;
        else if (leavingTeam === 'green') this.livesGreen = 0;
        else if (leavingTeam === 'yellow') this.livesYellow = 0;
        ...
        if (this.suddenDeathActive) {
          ...
          if (this.activeSuddenDeathTeams.length === 1) {
            const winnerTeam = this.activeSuddenDeathTeams[0];
            console.log(
              '[PongGM] Player left - Sudden death winner by default: ' +
                winnerTeam,
            );
            this.currentState = 'GAME_OVER';
            this.suddenDeathActive = false;
            ...
            const winnerName = this.config.getPlayerNameForTeam(
              winnerTeam,
              this,
            );
            const winnerText = this.config.uiText.suddenDeathWinner.replace(
        ...
        if (this.teamRed.length > 0) remainingTeams++;
        if (this.teamBlue.length > 0) remainingTeams++;
        if (this.teamGreen.length > 0) remainingTeams++;
        if (this.teamYellow.length > 0) remainingTeams++;

        // If only 1 team left, they win
        if (remainingTeams <= 1) {
          console.log('[PongGM] Only one team left - ending game');
          let winnerTeam = '';
          if (this.teamRed.length > 0) winnerTeam = 'red';
          else if (this.teamBlue.length > 0) winnerTeam = 'blue';
          else if (this.teamGreen.length > 0) winnerTeam = 'green';
          else if (this.teamYellow.length > 0) winnerTeam = 'yellow';
        ...
        console.log(
          '[PongGM] Game continues with ' + remainingTeams + ' teams',
        );
        return;
      }
    }
  ...
  */
   ...
   */
  onEvent_pong_startGame(eventData: { playerId: number }) {
    if (!playerManager.isHost) return;
   ...
   */
  onEvent_pong_debugSuddenDeath(eventData: { playerId: number }) {
    if (!playerManager.isHost) return;

    // Only host can trigger debug
    if (eventData.playerId !== playerManager.getMyPlayerId()) return;
    ...
    if (playerCount !== 4) {
      console.error(
        '[PongGM] DEBUG: Need exactly 4 players, have ' + playerCount,
      );
      return;
    }
    ...
    this.roundWins.red = 1 as any;
    this.roundWins.blue = 1 as any;
    this.roundWins.green = 1 as any;
    this.roundWins.yellow = 0 as any;

    console.log(
      '[PongGM] DEBUG: Round wins set - red:1, blue:1, green:1, yellow:0',
    );
    ...
    console.log(
      '[PongGM] DEBUG: Round=3, Lives=1 each, Wins: red=1, blue=1, green=1, yellow=0',
    );
    console.log(
      '[PongGM] DEBUG: After round 3 ends, red/blue/green will have 2 wins each → triggers sudden death!',
    );
  }
  ...
  onSpriteCollisionStart({
    ...
    if (this.config.roundMode !== 'button') return;
    ...
    const playerSprite =
      s1.playerId != undefined ? s1 : s2.playerId != undefined ? s2 : null;
    let zoneSprite = null;
  ...
  checkCampingViolations() {
    if (!playerManager.isHost) return;
    if (this.currentState !== 'ACTIVE') return;
    if (this.config.roundMode !== 'button') return; // Only enforce camping in button mode (Round 1)
    ...
    for (let i = 0; i < allPids.length; i++) {
      ...
      if (this.campingTimers[pidStr] && this.campingTimers[pidStr] > 0) {
        ...
        if (campingDuration >= this.config.campingTimeoutMs) {
          ...
          if (teamId) {
            const paddleInfo = this.config.getPaddlePosition(
              teamId,
              this.activePlayers.length,
            );
