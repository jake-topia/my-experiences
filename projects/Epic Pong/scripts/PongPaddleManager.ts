class PongPaddleManager extends SystemScript {
  ...
  movingSpriteIds: PseudoList;
  movingDirections: PseudoList;
  paddleSpriteIds: PseudoList;
  buttonSpriteIds: PseudoList;

  // Body Paddle mode tracking
  bodyPaddlePlayerIds: PseudoList; // Player IDs using body paddle mode
  bodyPaddleTeams: PseudoMap; // playerId -> teamId
  gameManager: any; // Reference to PongGameManager

  // Button flash animation state
  buttonFlashActive: boolean; // Is flash animation currently running

  hasId(list: PseudoList, id: string): boolean {
  ...
  addTrackedId(list: PseudoList, id: string) {
    if (!list) return;
    if (this.hasId(list, id)) return;
    list.push(id);
  }

  removeTrackedId(list: PseudoList, id: string): PseudoList {
    if (!list) return [] as any;
    const next: PseudoList = [] as any;
    let idx = 0;
    while (idx < list.length) {
      if (list[idx] !== id) {
        next.push(list[idx]);
  ...
  findIndex(list: PseudoList, id: string): number {
    if (!list) return -1;
    let idx = 0;
    while (idx < list.length) {
      if (list[idx] === id) {
  ...
  removeMovingSprite(spriteId: string) {
    const nextIds: PseudoList = [] as any;
    const nextDir: PseudoList = [] as any;
    let idx = 0;
  ...
  onInit() {
    ...
    this.movingSpriteIds = [] as any;
    this.movingDirections = [] as any;
    this.paddleSpriteIds = [] as any;
    this.buttonSpriteIds = [] as any;
    this.bodyPaddlePlayerIds = [] as any;
    this.bodyPaddleTeams = {} as PseudoMap;
   ...
   */
  createPaddle(playerId: number, teamId: string, playerCount: number) {
    ...
    const borderSpriteId =
      'paddle_border_' + playerId + '_' + (isHorizontal ? 'h' : 'v');
    ...
    this.paddleSpriteIds = this.removeTrackedId(
    ...
    const isBodyPaddleMode =
      roundMode === 'bodypaddle' || this.config.bodyPaddleMode;
  ...
  createControlButtons(
    ...
    console.log(
      '[PongPM] Created control buttons for player',
      playerId,
      'orientation:',
      orientation,
    );
   ...
   */
  onButtonPress(playerId: number, direction: string) {
    if (!playerManager.isHost) return;
   ...
   */
  updateButtonColors(playerId: number, activeDirection: string | null) {
    if (!playerManager.isHost) return;
  ...
  onPhysicsStep() {
    ...
    const nextIds: PseudoList = [] as any;
    const nextDirections: PseudoList = [] as any;
    let idx = 0;
    while (idx < this.movingSpriteIds.length) {
  ...
  updateCoordinatePaddles() {
    ...
    const activePlayers = this.gameManager.activePlayers || ([] as any);
    const playAreaX = this.config.playAreaX;
    const playAreaY = this.config.playAreaY;
    const playAreaWidth = this.config.playAreaWidth;
    const playAreaHeight = this.config.playAreaHeight;
    ...
    for (let i = 0; i < activePlayers.length; i++) {
      ...
      const layoutPlayerCount =
        this.gameManager.originalPlayerCount || activePlayers.length;
      ...
      const currentPaddleX =
        spriteManager.getProperty(spriteId, 'positionX') || paddleInfo.x;
      const currentPaddleY =
        spriteManager.getProperty(spriteId, 'positionY') || paddleInfo.y;
  ...
  updateBodyPaddles() {
    ...
    for (let i = 0; i < this.bodyPaddlePlayerIds.length; i++) {
      ...
      const paddleX =
        playerDetails.x - width / 2 + this.config.bodyPaddleOffsetX;
      const paddleY =
        playerDetails.y - height / 2 + this.config.bodyPaddleOffsetY;
  ...
  updateMovingPaddlePosition(spriteId: string, direction: number): boolean {
   ...
   */
  checkPaddleCollision(ballX: number, ballY: number, ballSize: number): any {
    let idx = 0;
    while (idx < this.paddleSpriteIds.length) {
      const spriteId = this.paddleSpriteIds[idx];
      const collision = this.checkCollisionForPaddle(
        spriteId,
        ballX,
        ballY,
        ballSize,
  ...
  checkCollisionForPaddle(
    spriteId: string,
    ballX: number,
    ballY: number,
    ballSize: number,
  ) {
    if (!spriteId) return { hit: false };
    const sprite = spriteManager.getSprite(spriteId);
    if (!sprite) return { hit: false };
   ...
   */
  removePaddleForPlayer(playerId: number) {
    if (!playerManager.isHost) return;
    ...
    this.bodyPaddlePlayerIds = this.removeTrackedId(
  ...
  removeButtonsForPlayer(playerId: number) {
    const targets = [
      'btn_left_' + playerId,
      'btn_right_' + playerId,
      'txt_left_' + playerId,
   ...
   */
  onEvent_pong_paddlePress(eventData: { playerId: number; direction: string }) {
    if (!playerManager.isHost) return;
  ...
  flashControlButtons() {
    ...
    console.log(
      '[PongPM] Button flash animation started (' +
        buttonRects.length +
        ' buttons)',
    );
  }
   ...
   */
  stopButtonFlash() {
    if (!this.buttonFlashActive) return;
   ...
   */
  updatePaddleBorderColors(backgroundColor: string) {
    if (!playerManager.isHost) return;
