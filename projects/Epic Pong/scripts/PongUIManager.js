...
class PongUIManager extends SystemScript {
    ...
    scoreLayoutStartX;
    scoreLayoutSpacing;
    playAreaSpriteId; // Central game board sprite
    currentPlayAreaColor; // Track current background color
    teamLaneSpriteIds; // Team lane rectangles (e.g., { red: 'lane_red', blue: 'lane_blue' })
    // --- Lives Display (NEW) ---
    livesSpriteIds; // teamId -> sprite ID for lives display
    // --- Win Count Display (NEW) ---
    winCountSpriteIds; // teamId -> sprite ID for win count display
    // --- Round Summary (NEW - Phase 5) ---
    roundSummarySprites; // Sprite IDs for round summary display
    // --- Round Celebration & Countdown (NEW) ---
    roundCelebrationSprites; // Sprite IDs for round celebration
    roundCountdownSprites; // Sprite IDs for round countdown
    // --- Sudden Death Elimination Popup ---
    suddenDeathElimSprites; // Sprite IDs for elimination popup
    onInit() {
        ...
        this.gameOverSprites = [];
        this.activeScoreTeams = [];
    ...
    createPlayArea() {
        ...
        console.log('[PongUI] Created play area sprite at (' +
    ...
    createTeamLanes(playerCount) {
        ...
        if (playerCount === 2) {
            ...
            this.createLaneRect('blue', 0, this.config.worldHeight - laneWidth, this.config.worldWidth, laneWidth);
        }
            ...
            this.createLaneRect('yellow', this.config.worldWidth - laneWidth, laneWidth, laneWidth, playAreaHeight);
        }
        console.log('[PongUI] Created team lane rectangles for ' + playerCount + ' players');
    }
    /**
     * Create a single lane rectangle for a team
     */
    createLaneRect(teamId, x, y, width, height) {
        if (!playerManager.isHost)
            return;
     ...
     */
    updateTeamSelection(gameManager) {
        if (!playerManager.isHost)
            return;
    ...
    hideLobby() {
        if (!playerManager.isHost)
            return;
        console.log('[PongUI] Hiding lobby - removing ' +
    ...
    showScoreboard(gameManager) {
        ...
        const activeTeams = [];
        if (gameManager.teamRed.length > 0)
            activeTeams.push('red');
        if (gameManager.teamBlue.length > 0)
            activeTeams.push('blue');
        if (gameManager.teamGreen.length > 0)
            activeTeams.push('green');
        if (gameManager.teamYellow.length > 0)
     ...
     */
    updateScoreboard(scoreRed, scoreBlue, scoreGreen, scoreYellow) {
        if (!playerManager.isHost)
            return;
        if (!this.activeScoreTeams || this.activeScoreTeams.length === 0)
            return;
        const teamScores = {
            red: scoreRed,
            blue: scoreBlue,
    ...
    getScoreSpriteId(teamId) {
        if (teamId === 'red')
            return this.scoreRedSpriteId;
        if (teamId === 'blue')
            return this.scoreBlueSpriteId;
        if (teamId === 'green')
            return this.scoreGreenSpriteId;
        if (teamId === 'yellow')
            return this.scoreYellowSpriteId;
        return '';
    }
    setScoreSpriteId(teamId, spriteId) {
        if (teamId === 'red')
            this.scoreRedSpriteId = spriteId;
        else if (teamId === 'blue')
            this.scoreBlueSpriteId = spriteId;
        else if (teamId === 'green')
            this.scoreGreenSpriteId = spriteId;
        else if (teamId === 'yellow')
            this.scoreYellowSpriteId = spriteId;
    }
    isTeamActive(teamId) {
        if (!this.activeScoreTeams)
            return false;
        // eslint-disable-next-line sonarjs/prefer-for-of -- Topia runtime lacks Symbol.iterator support
        for (let i = 0; i < this.activeScoreTeams.length; i++) {
    ...
    computeScoreX(teamId, text) {
        const index = this.getActiveTeamIndex(teamId);
        if (index === -1)
            return this.config.centerX;
        const scoreWidth = text.length * 9;
        return (this.scoreLayoutStartX + index * this.scoreLayoutSpacing - scoreWidth / 2);
    }
    renderOrUpdateScore(teamId, text) {
        if (!this.isTeamActive(teamId)) {
            this.removeScoreSprite(teamId);
            return;
        }
        ...
        try {
            ...
            console.error('[PongUI] Error updating score sprite ' + spriteId + ':', error);
        }
    }
    getTeamScore(gameManager, teamId) {
        if (!gameManager)
            return 0;
        if (teamId === 'red')
            return gameManager.scoreRed || 0;
        if (teamId === 'blue')
            return gameManager.scoreBlue || 0;
        if (teamId === 'green')
            return gameManager.scoreGreen || 0;
     ...
     */
    updateLivesDisplay(gameManager) {
        if (!playerManager.isHost)
            return;
        // Get active teams
        const activeTeams = [];
     ...
     */
    renderOrUpdateLives(teamId, lives, playerCount) {
        if (!playerManager.isHost)
            return;
     ...
     */
    getTeamLives(gameManager, teamId) {
        if (!gameManager)
            return 0;
        if (teamId === 'red')
            return gameManager.livesRed || 0;
        if (teamId === 'blue')
            return gameManager.livesBlue || 0;
        if (teamId === 'green')
            return gameManager.livesGreen || 0;
     ...
     */
    showGameOver(winnerName, gameManager) {
        if (!playerManager.isHost)
            return;
     ...
     */
    showRoundSummary(winnerText, nextRound, gameManager) {
        if (!playerManager.isHost)
            return;
        ...
        console.log('[PongUI] Round summary displayed with ' +
     ...
     */
    renderOrUpdateWinCount(teamId, wins, playerCount) {
        if (!playerManager.isHost)
            return;
     ...
     */
    showSuddenDeathElimination(playerName) {
        if (!playerManager.isHost)
            return;
        ...
        this.suddenDeathElimSprites = [bgId, elimId];
    ...
    hideSuddenDeathElimination() {
        if (!playerManager.isHost)
            return;
        if (!this.suddenDeathElimSprites)
            return;
        let index = 0;
     ...
     */
    showRoundCelebration(winnerText, roundNumber) {
        if (!playerManager.isHost)
            return;
        ...
        if (roundNumber === 3) {
            ...
            titleText = this.config.uiText.roundComplete.replace('{round}', roundNumber.toString());
        }
     ...
     */
    showRoundCountdown(roundNumber, secondsRemaining) {
        if (!playerManager.isHost)
            return;
        ...
        if (roundNumber === 2) {
            ...
            const buttonExtraWidth = buttonExtraText.length * buttonExtraFontSize * 0.5;
            spriteManager.addSprite('text', {
     ...
     */
    updateRoundCountdownNumber(secondsRemaining) {
        if (!playerManager.isHost)
            return;
        const countId = 'round_countdown_number';
