interface BonusZoneMovementPattern {
  name: string;
  initialX: number;
  initialY: number;
  initialSize: number;
...
class BonusZoneManager_Lava extends SystemScript {
  ...
  // --- Zone state ---
  zoneSprite: PseudoSprite;
  zoneAssetKey: string;
  private currentZoneId: string;

  // --- Patterns / motion ---
  private movementPatterns: PseudoList; // pseudo list style container (kept dynamic for platform leniency)
  private currentPattern: any;
  private direction: PseudoMap; // simple {dx, dy} vector (randomized per spawn)
  private pulseState: any;
  private movementTiming: number;
  private timingTracker: number;
  private animation: any;


  // --- Timers ---
  private pointInterval: number;
   ...
   */
  selectRandomPattern(): BonusZoneMovementPattern | null {
        if (!playerManager.isHost) return;

    if (this.movementPatterns.length === 0) return null;
  ...
  moveBonusZoneNowWithDuration(customDuration: number) {
        if (!playerManager.isHost) return;
  ...
  onPhysicsStep(deltaTime: number) {
    if (!playerManager.isHost) return;
    if (
      this.gameManager.currentGameState !== 'ACTIVE' ||
      this.currentZoneId == ''
    )
      return;
    this.timingTracker++;
   ...
   */
  isBonusZoneSprite(spriteId: string): boolean {
        if (!playerManager.isHost) return;
  ...
  // Collision hooks removed: GameManager_Lava handles collisions globally.
}
