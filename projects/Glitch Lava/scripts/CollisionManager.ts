class CollisionManager extends SystemScript {
  ...
  // --- Tracking ---
  players: PseudoList; // playerIds with colliders
  colliders: PseudoList; // collider uniqueIds aligned with players[]
  buffer: number;
  ...
  spawnCollider(playerId: number) {
    ...
    try {
      ...
      if (colliderSprite) {
        ...
        console.log(
          '[Collide] ERROR addSprite returned null for player=' + playerId,
        );
      }
    } catch (error) {
      console.log(
        '[Collide] ERROR spawnCollider failed for player=' + playerId,
        error,
      );
    }
  }
  ...
  onPhysicsStep() {
    ...
    if (this.buffer % 3 === 0) {
      for (let i = 0; i < this.players.length; i++) {
        ...
        if (currentIds.indexOf(this.players[i]) === -1) continue;
