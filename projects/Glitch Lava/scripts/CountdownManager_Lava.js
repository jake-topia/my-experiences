...
class CountdownManager_Lava extends SystemScript {
    ...
    onInit() {
        ...
        this.countdownTextSprite = spriteManager.addSprite('text', countdownSpriteOptions);
    }
    startCountdown(durationSeconds, eventName) {
    ...
    scheduleTick(nextValue) {
        //if (!playerManager.isHost) return;
        this.timerManager.setTimer('lavaGameCountdownTick', 1, {
            value: nextValue,
        });
    }
    onEvent_lavaGameCountdownTick(payload) {
        //if (!playerManager.isHost) return;
        if (payload.value > 0) {
