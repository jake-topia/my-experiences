class GameManagerLava extends SystemScript {
  ...
  centerSpawnY: number;
  startZoneId: string;
  startSpriteUniqueId: string;
  // --- Player Tracking ---
  readyPlayerIdsList: PseudoList; // players who stood on start pad
  activePlayerIds: PseudoList; // snapshot at game start; used for scoring
  numberOfPlayersInGame: number;

  // --- Game State / Scoring ---
  currentGameState: string; // WAITING_FOR_PLAYERS | PRE_COUNTDOWN | COUNTDOWN | ACTIVE | GAME_OVER
  cyclesSurvived: number; // waves started (score uses waves completed)
  totalScore: number;

  // --- Timing Config (read by subsystems) ---
  countdownDuration: number;
  flashDurationPerWarning: number;
  numberOfFlashes: number;
  lavaActiveDurationPerWave: number;
  pauseBetweenWaves: number;
  lobbyIntroMessage: string;

  // --- Subsystems (attached in onInit) ---
  lavaMapManager: any;
  scoreManager: any;
  timerManager: any;
  countdownManager: any;
  highScoreManager: any;
  bonusZoneManager: any;
  collisionManager: any;
  particleManager: any;
  // Debug: did we finish attaching subsystems in onInit?
  didAttachSubsystems: boolean;
  // Queue player joins that arrive before subsystems are ready
  pendingPlayerJoins: PseudoList;

  // --- Phase System ---
  phaseEnabled: boolean; // master toggle
  currentPhaseId: string; // '' when not in a phase
  phaseEndWave: number; // wave index when current phase ends
  nextPhaseWave: number; // wave index when the next phase may start
  baseHue: number; // starting hue for visual drift
  currentHue: number; // current hue applied to visuals
  phaseDefs: PseudoList; // array of phase definitions (id, name, duration, colorShift, applyMethod)
  specialInterval: number;
  // --- Analytics ---
  analyticsPublicKey: string;
  lastGameInitiatorProfileId: string;
  // --- Leveling & Testing ---
  level: number;
  levelInterval: number; // waves per level
  nextLevelWave: number;
  levelBonusPoints: number;
  levelBonusPerLevel: number; // added: additional bonus grows each level
  // --- Centralized difficulty scaling ---
  speedMultiplierBase: number; // base speed multiplier applied to moving systems (<=1.0 means slower than base)
  speedMultiplierPerLevel: number; // added per level (level-1)
  speedMultiplierMin: number; // clamp low bound
  speedMultiplierMax: number; // clamp high bound
  warningSecondsBase: number; // base warning flash seconds at level 1
  warningSecondsPerLevel: number; // subtracted per level (level-1)
  warningSecondsMin: number; // clamp min seconds
  testingMode: boolean;
  testingSequence: PseudoList; // array of { kind: 'static'|'special', index?:number, methodName?:string }
  testingIndex: number;
  // --- Visual Overlay (stage tint substitute) ---
  overlayAssetId: string; // configurable asset id for full-screen panel
  overlaySpriteId: string;
  // --- Level-up UX ---
  levelPauseSeconds: number; // extra pause after level completion before next wave starts
  lastAnnouncedLevelCompleted: number; // tracks last completed level we've shown a banner for
  ...
  async onInit() {
    ...
    this.pendingPlayerJoins = [] as any;
    // Analytics setup
    this.analyticsPublicKey =
      (stateManager.getVariable('PublicKey') as any) || '';
    if (playerManager.isHost) this.setWorldActivity('GAME_WAITING');
    ...
    this.readyPlayerIdsList = [] as any;
    this.activePlayerIds = [] as any;
    ...
    this.overlayAssetId =
      (stateManager.getVariable &&
        (stateManager.getVariable('OverlayAssetId') as any)) ||
      'overlay_rect';
    ...
    if (
      ...
      this.particleManager.displayParticles(
        { x: this.centerSpawnX, y: this.centerSpawnY },
        2, //
        5, // seconds
      );
    }
    ...
    if (this.testingMode) {
      try {
        ...
        console.log(
          '[GM] INFO testingMode enabled. Items=',
          (this.testingSequence && this.testingSequence.length) || 0,
        );
      } catch (e) {
  ...
  getSpeedMultiplier(): number {
    ...
    let mul =
      (this.speedMultiplierBase || 1) +
      Math.max(0, lvl - 1) * (this.speedMultiplierPerLevel || 0);
    const lo =
      typeof this.speedMultiplierMin === 'number'
        ? this.speedMultiplierMin
        : 0.3;
    const hi =
      typeof this.speedMultiplierMax === 'number'
  ...
  getTimeScale(): number {
    ...
    if (!speed || speed <= 0) return 1.0;
    let scale = 1.0 / speed;
    // Keep within reasonable bounds
    if (scale < 0.33) scale = 0.33; // limit to 3x speed-up
    if (scale > 3.0) scale = 3.0; // limit to 3x slow-down
    return scale;
  }
  ...
  getWarningSeconds(): number {
    const lvl = this.getCurrentLevel();
    const base =
      typeof this.warningSecondsBase === 'number'
        ? this.warningSecondsBase
        : 3.0;
    const step =
      typeof this.warningSecondsPerLevel === 'number'
        ? this.warningSecondsPerLevel
        : 0.25;
    const minS =
      typeof this.warningSecondsMin === 'number' ? this.warningSecondsMin : 1.5;
    let sec = base - Math.max(0, lvl - 1) * step;
    if (sec < minS) sec = minS;
    return sec;
  }
  ...
  drawOrUpdateStartButton() {
    ...
    try {
      ...
      if (flag === '0' || flag === false) showTestingBtn = false;
    } catch {}
  ...
  removeStartButton() {
    ...
    if (spriteManager.getSprite(testId)) spriteManager.removeSprite(testId);
  }
  ...
  setupStartZone() {
    if (!playerManager.isHost) return;
    const startSprite: PseudoSprite | null = spriteManager.addSprite(
      'start_pad_asset',
      {
    ...
    console.log(
      `[GM] INFO setupStartZone: startSpriteId='${this.startSpriteUniqueId}'`,
    );
  }
  ...
  checkIfAllPlayersReady() {
    ...
    for (let i = 0; i < readyArray.length; i++) {
      ...
      if (currentPlayersInRoom.indexOf(pid) !== -1) nextReadyList.push(pid);
    }
    this.readyPlayerIdsList = nextReadyList as any;
  ...
  onPlayerJoined({ playerId }: { playerId: number }) {
    ...
    try {
      ...
      playerManager.setNameplate(
        playerId,
        `🔥 ${details ? details.username : 'Player ' + playerId} 🔥`,
      );
    } catch (e) {
    ...
    if (this.currentGameState === 'WAITING_FOR_PLAYERS') {
      ...
      console.log(
        `[GM] INFO playerJoined: spectating (state=${this.currentGameState})`,
      );
    }
        ...
        if (profileId) {
          integrationsManager.putPublicKeyAnalytics({
            ...
            analytics: [
              ...
              {
                analyticName:
                  'lavaNumPlayersOnJoin-' + playerManager.getPlayerIds().length,
              },
            ],
          });
        }
  ...
  safeAddCollider(playerId: number) {
    if (!playerManager.isHost) return;
    ...
    if (typeof this.collisionManager.addPlayer !== 'function') {
      console.error(
        '[GM] ERROR safeAddCollider: addPlayer not a function; keys=',
        Object.keys(this.collisionManager).join(', '),
      );
      return;
    }
    try {
  ...
  onPlayerLeft({ playerId }: { playerId: number }) {
    ...
    const nextReadyList = [] as any;
    // eslint-disable-next-line prefer-for-of
    for (let i = 0; i < readyArray.length; i++) {
      const rid = readyArray[i];
      if (rid !== playerId) nextReadyList.push(rid);
    }
    ...
    if (this.currentGameState === 'WAITING_FOR_PLAYERS') {
      if (wasReady) this.checkIfAllPlayersReady();
      return;
    }
    ...
    if (this.currentGameState === 'ACTIVE' && wasActivePlayer) {
      if (this.activePlayerIds.length === 0) {
        ...
        console.log(
          `[GM] INFO active: player ${playerId} left, remaining=${this.activePlayerIds.length}`,
        );
      }
    }
  }
  ...
  onSpriteCollisionStart({
    ...
    if (!playerManager.isHost) return;
    if (this.currentGameState === 'GAME_OVER') return;
    ...
    if (this.currentGameState === 'ACTIVE') {
      ...
      if (!s1_isPlayer && !s2_isPlayer) return;
      ...
      if (this.lavaMapManager.isLavaSprite(objectSprite.uniqueId)) {
        ...
        this.bonusZoneManager.isBonusZoneSprite(objectSprite.uniqueId)
      ) {
  ...
  onEvent_triggerCountdownNow() {
    //if (!playerManager.isHost) return;
    if (this.currentGameState !== 'PRE_COUNTDOWN') return;
  ...
  onEvent_startGameplayAfterCountdown() {
    ...
    if (
      ...
      console.log(
        `[GM] WARN startGameplayAfterCountdown in state=${this.currentGameState}`,
      );
    }
    ...
    this.scoreManager.updateLiveScore(
      this.cyclesSurvived,
      this.numberOfPlayersInGame,
    );
   ...
   */
  onEvent_startGame(eventData: { playerId: number; fromPlayerId?: number }) {
    if (!playerManager.isHost) return;
    if (this.currentGameState !== 'WAITING_FOR_PLAYERS') return;
    ...
    if (this.analyticsPublicKey) {
      try {
        ...
        const starterProfile =
          playerManager.getPlayerDetails(starterPid).profileId;
        if (starterProfile) this.lastGameInitiatorProfileId = starterProfile;
        const analytics: any[] = [{ analyticName: 'lavaGameStarts' }];
        if (starterProfile) {
          analytics.push({
  ...
  startNextWave() {
    ...
    const completedCycles =
      this.cyclesSurvived > 0 ? this.cyclesSurvived - 1 : 0;
    ...
    if (lastName) this.scoreManager.displayMessage(lastName);
  }
  ...
  applyTestingPatternForCurrentIndex(): boolean {
    if (!playerManager.isHost) return false;
    if (!this.lavaMapManager) return false;
    var list = (this.testingSequence && (this.testingSequence as any)) || [];
  ...
  onEvent_lavaWaveActive() {
    ...
    this.timerManager.setTimer(
      'endCurrentLavaWave',
      this.lavaActiveDurationPerWave,
    );
  }
  ...
  onEvent_endCurrentLavaWaveNow() {
    ...
    if (this.levelInterval > 0) {
      var completedLevels = Math.floor(
        this.cyclesSurvived / this.levelInterval,
      );
      if (completedLevels > this.lastAnnouncedLevelCompleted) {
        ...
        try {
          ...
          this.timerManager.setTimer(
            'overlayFlashOff',
            Math.min(0.75, this.levelPauseSeconds),
          );
        } catch (e) {
   ...
   */
  /* eslint-disable-next-line sonarjs/cognitive-complexity */
  onEvent_playerHitLava(eventData: { playerId: number; lavaSpriteId: string }) {
    if (!playerManager.isHost) return;
    if (this.currentGameState === 'GAME_OVER') return;
    if (this.currentGameState !== 'ACTIVE') {
    ...
    console.log(
      '[GM] DEBUG: About to submit score. Player ID list type: ' +
        typeof this.activePlayerIds,
    );
    console.log(
      '[GM] DEBUG: Player IDs to submit: ' + JSON.stringify(playersToSubmit),
    );
    console.log(
      '[GM] DEBUG: Number of players found: ' + playersToSubmit.length,
    );
      ...
      try {
        const participants =
          ...
          (this.activePlayerIds.toArray
            ? this.activePlayerIds.toArray()
            : this.activePlayerIds);
        const analytics: any[] = [{ analyticName: 'lavaGameCompletions' }];
        ...
        if (this.lastGameInitiatorProfileId) {
          analytics.push({
            analyticName:
              'lavaLastInitiator-' + this.lastGameInitiatorProfileId,
          });
        }
        integrationsManager.putPublicKeyAnalytics({
          interactivePublicKey: this.analyticsPublicKey,
  ...
  checkPhaseLifecycleAndApply(): boolean {
    if (!playerManager.isHost) return false;
    if (!this.phaseEnabled) return false;
   ...
   */
  applyCurrentPhasePattern(): boolean {
    if (!playerManager.isHost) return false;
    if (!this.lavaMapManager) return false;
   ...
   */
  applyHueToWorld(hueValue: number) {
  ...
  setOverlayEnabled(
    enabled: boolean,
    opts?: { opacity?: number; color?: string; assetId?: string },
  ) {
    if (!playerManager.isHost) return;
    const id = this.overlaySpriteId;
    const asset =
      (opts && opts.assetId) || this.overlayAssetId || 'overlay_rect';
    const opacity =
      opts && typeof opts.opacity === 'number' ? opts.opacity : 0.12;
  ...
  getRandomPhaseDef(): any {
    if (!this.phaseDefs || this.phaseDefs.length === 0) return null;
    const idx = Math.floor(Math.random() * this.phaseDefs.length);
    return this.phaseDefs[idx];
  }

  getPhaseDefById(id: string): any {
    if (!this.phaseDefs || this.phaseDefs.length === 0) return null;
