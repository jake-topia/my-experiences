...
class HighScoreManager_Lava extends SystemScript {
    ...
    async onInit() {
        ...
        if (spriteManager && typeof spriteManager.addSprite === 'function') {
            ...
            console.log('[HighScore] WARN could not create score sprite, spriteManager unavailable.');
        }
     ...
     */
    buildTeamInitials(playerIdsArray) {
     ...
     */
    async submitScore(playerIds, cyclesSurvived, totalScore) {
    ...
    // --- Cloud persistence ----------------------------------------------------
    getPublicKey() {
        var key = stateManager.getVariable('PublicKey');
        if (!key) {
            console.log('!!! HighScoreManager_Lava: PublicKey missing; cannot load/save.');
            return null;
        }
        return key;
    }
    async loadHighScores() {
        ...
        console.log('[HighScore] INFO loaded ' +
    ...
    async saveHighScores() {
        ...
        console.log('[HighScore] INFO saved ' + toSave.length + ' entries to cloud.');
    }
