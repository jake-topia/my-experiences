...
interface LavaWavePattern {
  name: string;
  zones: LavaZoneDef[];
  // Level gating
  startingLevel?: number; // defaults to 1 if undefined
  endingLevel?: number; // defaults to a large number if undefined
}

class LavaMapManager extends SystemScript {
  // --- Pattern / State ---
  wavePatterns: PseudoList;
  currentWavePattern: any;
  private _isCleaningUp: boolean;
  flashingSprites: PseudoList; // pre-lava warning visuals
  activeLavaSprites: PseudoList; // dangerous, collidable lava
  ...
  patternBag: PseudoList; // array of indices into wavePatterns
  patternBagIndex: number;

  // --- Internal counters ---
  private currentFlashCount: number;

  // --- Phase pending state (used instead of closure callbacks) ---
  private pendingPhaseType: string; // '', 'creep', 'sweep', 'ripple1'|'ripple2'|'ripple3'
  private pendingWaveKey: number; // wave index captured when scheduling
  ...
  defineWavePatterns() {
    ...
    try {
      ...
      for (var i = 0; i < arr.length; i++) {
        var pat = arr[i] as LavaWavePattern;
        if (pat && typeof pat.startingLevel !== 'number') pat.startingLevel = 1;
        if (pat && typeof pat.endingLevel !== 'number') pat.endingLevel = 9999;
      }
    } catch (e) {
  ...
  selectRandomPattern(): LavaWavePattern | null {
    ...
    if (pat && pat.name) this.lastPatternName = pat.name;
    return pat;
  }
  ...
  getPatternCount(): number {
    try {
      return this.wavePatterns && this.wavePatterns.toArray
        ...
        : (this.wavePatterns as any).length || 0;
    } catch (e) {
      return 0;
    }
  }

  setNextPatternIndex(idx: number) {
    // Force the next static pattern by resetting the pattern bag to start with idx
    try {
      if (typeof idx !== 'number' || idx < 0) return;
      var total =
        this.wavePatterns && this.wavePatterns.toArray
          ? this.wavePatterns.toArray().length
          : (this.wavePatterns as any).length || 0;
      if (idx >= total) return;
      this.patternBag = [idx] as any;
      this.patternBagIndex = 0;
    } catch (e) {
      // no-op
    }
  ...
  prepareNextWave() {
    ...
    for (var i = 0; i < this.currentWavePattern.zones.length; i++) {
      ...
      if (flashSprite) this.flashingSprites.push(flashSprite);
    }
  ...
  onEvent_lavaFlashTickNow() {
    if (this.pendingPhaseType && this.pendingPhaseType !== '') {
      ...
      if (this.pendingPhaseType === 'creep') {
        ...
        if (top) this.activeLavaSprites.push(top);
        if (bottom) this.activeLavaSprites.push(bottom);
        if (left) this.activeLavaSprites.push(left);
        if (right) this.activeLavaSprites.push(right);
        ...
        if (UL) this.activeLavaSprites.push(UL);
        if (UR) this.activeLavaSprites.push(UR);
        if (LL) this.activeLavaSprites.push(LL);
        if (LR) this.activeLavaSprites.push(LR);
        ...
        if (Hbar) this.activeLavaSprites.push(Hbar);
        if (Vbar) this.activeLavaSprites.push(Vbar);
      ...
      if (this.pendingPhaseType === 'ripple1') {
        ...
        if (a1) this.activeLavaSprites.push(a1);
      ...
      if (this.pendingPhaseType === 'ripple2') {
        ...
        this.activeLavaSprites = (
          this.activeLavaSprites && this.activeLavaSprites.toArray
            ? this.activeLavaSprites.toArray()
            : (this.activeLavaSprites as any)
        ).filter(function (s) {
        ...
        if (a2) this.activeLavaSprites.push(a2);
      ...
      if (this.pendingPhaseType === 'ripple3') {
        ...
        if (a3) this.activeLavaSprites.push(a3);
      ...
      if (this.pendingPhaseType === 'clamp') {
        ...
        if (t) this.activeLavaSprites.push(t);
        if (b) this.activeLavaSprites.push(b);
        if (l) this.activeLavaSprites.push(l);
        if (r) this.activeLavaSprites.push(r);
      ...
      if (this.pendingPhaseType === 'corners') {
        ...
        if (tl) this.activeLavaSprites.push(tl);
        if (br) this.activeLavaSprites.push(br);

        // grow towards center to ~60% of short side
        var target = Math.floor((W < H ? W : H) * 0.6),
          cx = (W - target) / 2,
          cy = (H - target) / 2;
        if (tl)
          timerManager.animate({
      ...
      if (this.pendingPhaseType === 'lanes') {
        ...
        if (h1) this.activeLavaSprites.push(h1);
        if (h2) this.activeLavaSprites.push(h2);
        if (h3) this.activeLavaSprites.push(h3);
      ...
      if (this.pendingPhaseType === 'blink1') {
        ...
        var cellW = Math.floor(W / 3),
          cellH = Math.floor(H / 3);
        var ids = [];
  ...
  activateLavaAfterFlashing() {
    ...
    if (!this.currentWavePattern || !this.gameManager) return;
    ...
    for (var i = 0; i < this.currentWavePattern.zones.length; i++) {
      ...
      if (lavaSprite) this.activeLavaSprites.push(lavaSprite);
    }
  ...
  isLavaSprite(id: string): boolean {
    if (!id || id.indexOf('lava_') === -1) return false;
    if (
      ...
      if (grp && grp !== 'lava') return false; // ✅ only real lava
    }
    return true;
  }
  ...
  spawnCreepingLava() {
    ...
    var wk =
      this.gameManager && typeof this.gameManager.cyclesSurvived === 'number'
  ...
  buildPatternBag() {
    ...
    var lvl = 1;
    try {
      if (this.gameManager && typeof this.gameManager.level === 'number') lvl = this.gameManager.level;
    } catch (e) {}
    for (var i = 0; i < arr.length; i++) {
      var p = arr[i] as LavaWavePattern;
      var s = typeof p.startingLevel === 'number' ? p.startingLevel : 1;
      var e = typeof p.endingLevel === 'number' ? p.endingLevel : 9999;
      if (s <= lvl && lvl <= e) this.patternBag.push(i);
    }
    ...
    if (this.patternBag.length < minCount) {
      this.patternBag = [] as any;
      for (var k = 0; k < arr.length; k++) this.patternBag.push(k);
    }
  ...
  getWarningSeconds(): number {
    ...
    if (sec < 1.5) sec = 1.5; // clamp min
    return sec;
  }

  animateWarnOpacity(uniqueId: string) {
    ...
    var animDur = Math.floor(((this.gameManager && typeof this.gameManager.getWarningSeconds === 'function')
      ...
      : 3.0) * 1000);
    timerManager.animate({
      targets: [{ uniqueId: uniqueId } as any],
      keyframes: {
        0: { opacity: 0.15 },
        25: { opacity: 0.75 },
        50: { opacity: 0.15 },
