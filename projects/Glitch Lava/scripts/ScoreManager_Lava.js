...
class ScoreManagerLava extends SystemScript {
    // --- Sprite refs ---
    messageTextSprite; // lobby / status messages
    liveScoreTextSprite; // "SCORE: X"
    // --- Layout ---
    worldWidth;
    messagePositionX;
    messagePositionY;
    ...
    onInit() {
        ...
        try {
            ...
            const scoreSprite = spriteManager.addSprite('text', liveScoreSpriteOptions);
            if (scoreSprite) {
     ...
     */
    displayMessage(message, xPos, yPos) {
        ...
        try {
            const updateOptions = { text: message };
            if (xPos !== undefined) {
            ...
            spriteManager.updateSprite(this.messageTextSprite.uniqueId, updateOptions);
            if (this.timerManager) {
     ...
     */
    onEvent_clearMessageNow(payload) {
        if (payload?.spriteId &&
            ...
            try {
                ...
                console.log('[Score] ERROR message: clear failed for ' + payload.spriteId, e);
            }
        }
        else if (this.messageTextSprite &&
            payload?.spriteId !== this.messageTextSprite.uniqueId) {
            console.log('[Score] WARN clearMessageNow: mismatched spriteId=' + payload.spriteId);
        }
    }
    /**
     * Refresh the "SCORE: X" display.
     */
    updateLiveScore(cyclesCompleted, numPlayers) {
     ...
     */
    recordFinalScore(playerId, finalCycles, finalTotalScore) {
        console.log('[Score] INFO final: playerId≈' +
            playerId +
            ' cycles=' +
            finalCycles +
            ' total=' +
