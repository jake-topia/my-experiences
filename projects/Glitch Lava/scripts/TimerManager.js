...
class TimerManager extends SystemScript {
    ...
    gameplayUpdateIntervalTargetStep;
    resetLastTeleportedTimerTargetStep;
    resetLastTeleportedPayload;
    // --- Lava countdown tick (stringified payload {value}) ---
    lavaGameCountdownTick_TargetStep;
    lavaGameCountdownTick_Payload;
    // --- Lava game timers ---
    triggerCountdown_TargetStep;
    endCurrentLavaWave_TargetStep;
    initiateNextWaveCycle_TargetStep;
    offerRestart_TargetStep;
    lavaFlashTick_TargetStep;
    finishMainCountdown_TargetStep;
    // --- UI message clear ---
    messageClear_TargetStep;
    messageClear_Payload;
    // --- Bonus Zone timers ---
    moveBonusZone_TargetStep;
    awardBonusPoints_TargetStep;
    jumpBonusZone_TargetStep;
    constructor() {
     ...
     */
    setTimer(type, delaySeconds, payload) {
        var delaySteps = 1;
        if (delaySeconds > 0) {
            var wholeSteps = 0;
    ...
    setInterval(type, intervalSeconds) {
        ...
        console.log('[Timer] INFO setInterval gameplayUpdate every ' +
            intervalSteps +
            ' steps');
    }
    /**
