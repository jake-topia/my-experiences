...
class BallManager extends ComponentScript {
    ...
    lastPlayerTouch;
    hitBoxes = [];
    isBetweenPlays = false;
    hockeyPuckWidth;
    hockeyPuckHeight;
    // Puck possession state
    _possessingPlayerId;
    _possessingPlayerLookupId;
    _possessingStickId;
    _possessionStartMs;
    _possessionDurationMs;
    _initialSyncMovedOn;
    _initialMoveToX;
    _initialMoveToY;
    _possessionOffsetX;
    ...
    _clampToRink(px, py) {
        ...
        if (clamped_x < 100)
            clamped_x = 100;
        if (clamped_x > 2780 - this.hockeyPuckWidth)
            clamped_x = 2780 - this.hockeyPuckWidth;
        if (clamped_y < this._wallTop)
            clamped_y = this._wallTop;
        if (clamped_y > this._wallBottom)
            clamped_y = this._wallBottom;
    ...
    onPhysicsStep(deltaTime) {
        ...
        if (this._possessingPlayerId !== "") {
            var playerSprite = spriteManager.getSprite(String(this._possessingPlayerLookupId));
            if (!playerSprite) {
                this._firePuck(0, this.lowSpeed);
                return;
            }
            ...
            var hasMoved = playerSprite._syncMovedOn !== this._initialSyncMovedOn ||
        ...
        if (Math.abs(vx) > 0.5 || Math.abs(vy) > 0.5) {
            ...
            // Top-right corner
            else if (edgeRight - px + (py - wallTop) < ci &&
                px > edgeRight - ci &&
                py < wallTop + ci &&
                vy - vx < 0) {
                newVx = vy;
            ...
            // Bottom-left corner
            else if (px - edgeLeft + (wallBottom - py) < ci &&
                px < edgeLeft + ci &&
                py > wallBottom - ci &&
                vx - vy < 0) {
                newVx = vy;
            ...
            // Bottom-right corner
            else if (edgeRight - px + (wallBottom - py) < ci &&
                px > edgeRight - ci &&
                py > wallBottom - ci &&
                -vx - vy < 0) {
                newVx = -vy;
            ...
            if (bounced) {
                ...
                if (this.sprite.position.y < wallTop)
                    this.sprite.position.y = wallTop;
        ...
        const isTooFarLeft = this.sprite.position.x <= 0 - this.hockeyPuckWidth + 10;
        const isTooFarRight = rightSideX >= this.experienceWidth + this.hockeyPuckWidth - 10;
        const isTooFarUp = this.sprite.position.y <= 0 - this.hockeyPuckHeight + 10;
        const isTooFarDown = bottomSideY >= this.experienceHeight + this.hockeyPuckHeight - 10;
        if (isTooFarLeft || isTooFarRight || isTooFarUp || isTooFarDown) {
            ...
            var respawnX;
            var respawnY;
            var isCenter = this.sprite.position.x >= halfW * 0.75 &&
    ...
    onSpriteCollisionStart({ collisionX, collisionY, sprite }) {
        ...
        if (sprite.uniqueId.indexOf("ih_goalie_") !== -1) {
            ...
            if (holdingVar !== "" && holdingVar !== this.sprite.uniqueId) {
                ...
                if (gnLen > 0.001) {
                    var gnnx = gnx / gnLen;
                    var gnny = gny / gnLen;
                    var gdot = this.sprite.velocity.x * gnnx + this.sprite.velocity.y * gnny;
                    if (gdot < 0) {
            ...
            if (saveRoll >= 0.25) {
                if (saveRoll < 0.65) {
                    ...
                    if (bnLen > 0.001) {
                        var bnnx = bnx / bnLen;
                        var bnny = bny / bnLen;
                        var bdot = this.sprite.velocity.x * bnnx + this.sprite.velocity.y * bnny;
                        if (bdot < 0) {
            ...
            stateManager.setVariable("GoalieHolding_" + goalieTeamIdx, this.sprite.uniqueId);
            return;
        }
        ...
        if (sprite.uniqueId.indexOf("ih_stick_") !== -1) {
            ...
            if (sprite.uniqueId === this._immuneStickId &&
            ...
            if (this._possessingPlayerId === stickPlayerIdStr)
                return;
            // Cooldown — kicker can't re-catch their own shot
            if (stickPlayerIdStr === this._lastFiredPlayerId &&
        ...
        if (this.walls.includes(sprite.uniqueId) ||
            this.hitBoxes.includes(sprite.uniqueId)) {
    ...
    _getFormationPositions(teamIndex, count) {
    ...
    _startGoalCelebration(teamIndex) {
        this._goalScored = 1;
    ...
    _fireGoalParticles(teamIndex) {
        ...
        if (scoringPlayers.toArray)
            scoringPlayers = scoringPlayers.toArray();
        if (concedingPlayers.toArray)
            concedingPlayers = concedingPlayers.toArray();
    ...
    _goalieShoot() {
        ...
        if (!gameStarted) {
            ...
            if (playerIds && playerIds.toArray)
