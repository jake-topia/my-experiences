...
class PlayerMenuPanel extends SystemScript {
    ...
    onPhysicsStep(deltaTime) {
        ...
        var stateChanged = menuState !== this._lastMenuState ||
    ...
    _buildDetailText(menuState, actionText, detailText) {
        var explainer = "";
        if (menuState === "READY_TO_START") {
    ...
    _updateContent(menuState, actionText, detailText) {
        var toggleLabel = this._expanded ? this._copy.toggle.expanded : this._copy.toggle.collapsed;
        var fullDetailText = this._buildDetailText(menuState, actionText, detailText);
    ...
    _updatePositions(panelX, panelY) {
    ...
    _showSprite(uniqueId, opacityVal) {
    ...
    _toggleExpand() {
        ...
        this._updateContent(this._lastMenuState, this._lastActionText, this._lastDetailText);
        var myId = playerManager.getMyPlayerId();
        if (myId) {
            var details = playerManager.getPlayerDetails(myId);
            if (details) {
    ...
    _showModeToggle() {
        var label = this._controlScheme === "mouse" ? this._copy.mode.mouse : this._copy.mode.keyboard;
    ...
    onSpriteClicked({ event, sprite }) {
        ...
        if (sprite.uniqueId === "ih_action_bg" ||
        ...
        if (sprite.uniqueId === "ih_panel_bg" ||
            sprite.uniqueId === "ih_action_text" ||
