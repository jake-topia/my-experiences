...
class main extends SystemScript {
    ...
    onHostStart() {
        ...
        if (numPlayers >= 2) {
            this._setMenuState("READY_TO_START", "Click here to Start Game", "\uD83C\uDFD2 Free Skate\nYou can shoot pucks and practice.\nWe recommend using a mouse.");
            this.addScoreSprites();
        }
        else {
            this._setMenuState("WAITING_FOR_PLAYERS", "\uD83C\uDFD2 Free Skate", "This is ice hockey! You can shoot pucks and practice.\nWe recommend using a mouse.\nWhen another person joins, you can start a match!");
    ...
    onPlayerJoined({ playerId }) {
        ...
        if (stateManager.getVariable("GameStarted") === true) {
            ...
            if (assignTeam === 1) {
                ...
                playerManager.setNameplate(playerId, "\uD83C\uDFD2 " +
        ...
        if (numPlayers >= 2 && stateManager.getVariable("GameStarted") === false) {
            try {
                ...
                console.log("Main script failed in Getting num players and gamestarted variable for queue operations");
            }
        }
    ...
    async onEvent_playerStarted() {
        ...
        this._setMenuState("GAME_ACTIVE", "", "Score 5 goals first to win or have more goals than the other team before the timer runs out!");
        ...
        for (let i = 0; i < playersLength; i++) {
            ...
            if ((i + 1) % 2 !== 0) {
                ...
                playerManager.setNameplate(playerIds[i], "\uD83C\uDFD2 " +
    ...
    _spawnDemoPucks() {
        ...
        for (var pi = 0; pi < playerIds.length; pi++) {
            ...
            if (det)
                playerPos.push({ x: det.x, y: det.y });
        }
        ...
        for (var si = 0; si < scored.length - 1; si++) {
            ...
            for (var sj = si + 1; sj < scored.length; sj++) {
                if (scored[sj].dist < scored[minIdx].dist)
                    minIdx = sj;
            }
            if (minIdx !== si) {
                var tmp = scored[si];
    ...
    _setMenuState(state, actionText, detailText) {
    ...
    _showStartButton() {
        if (!playerManager.isHost)
            return;
        if (spriteManager.getSprite("ih_start_btn_bg"))
            return;
        spriteManager.addSprite("LeftWall", {
    ...
    onSpriteClicked({ event, sprite }) {
        if (!sprite)
            return;
        if (sprite.uniqueId === "ih_start_btn_bg" ||
    ...
    _sendCompletionAnalytics() {
        ...
        if (allPlayers && allPlayers.toArray)
