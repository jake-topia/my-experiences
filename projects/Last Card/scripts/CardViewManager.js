...
class CardViewManager extends SystemScript {
    ...
    myCardSpriteIds; // ids for my hand
    opponentCardSpriteIds; // (reserved) opponents if/when rendered
    persistentUiIds; // UI that survives updates (buttons, draw pile)
    _suitSelectorIds; // suit pick overlay sprites
    _playerStatusIds; // opponent status displays (name + card count)
    _textBgIds; // background rectangles for text elements
    _helpSpriteIds; // help overlay sprites
    _instructionSpriteIds; // lobby instruction sprites
    // ---------- Help drawer state ----------
    _helpDrawerOpen;
    // ---------- Camping Teleport Protection (TP) ----------
    _campingTpZoneId; // Sprite ID for camping TP zone
    _campingCheckCounter; // Frame counter for camping checks
    // ---------- Periodic sync verification ----------
    _syncCheckIntervalFrames;
    _framesSinceLastSync;
    _lastSyncRequestTime;
    _syncRequestCooldownMs;
    // ---------- Config provider for instructions ----------
    _configProvider;
    // ---------- Visual + local state mirrors ----------
    cardWidth;
    cardHeight;
    _localGameState; // last payload received (read-only usage)
    _lastStateVersion;
    ...
    _layoutConfig; // Reference to LayoutConfig system
    ...
    onInit() {
        ...
        console.log('[CVM] LayoutConfig.STAGE_WIDTH:', this._layoutConfig.STAGE_WIDTH);
        console.log('[CVM] LayoutConfig.STAGE_HEIGHT:', this._layoutConfig.STAGE_HEIGHT);
        ...
        console.log('[CVM] After assignment - cardWidth:', this.cardWidth, 'cardHeight:', this.cardHeight);
        console.log('[CVM] After assignment - stageWidth:', this.stageWidth, 'stageHeight:', this.stageHeight);
        console.log('[CVM] After assignment - centerX:', this.centerX, 'centerY:', this.centerY);
        ...
        this._playerStatusIds = [];
        console.log('[CVM] init _textBgIds');
        this._textBgIds = [];
        this._helpSpriteIds = [];
        this._instructionSpriteIds = [];
        ...
        if (this._framesSinceLastSync >= this._syncCheckIntervalFrames) {
            ...
            if (timeSinceLastRequest >= this._syncRequestCooldownMs) {
                ...
                // During active game, also request periodic sync to recover from missed updates
                    ...
                    console.log('[CVM] Periodic sync during active game v' + this._lastStateVersion);
                    this._lastSyncRequestTime = now;
                    eventManager.emit('playerRequestsFullSync', {
                        fromPlayerId: this.myPlayerId,
                    });
                ...
                // During suit selection, sync more aggressively to ensure clients stay in sync
                    ...
                    console.log('[CVM] Periodic sync during suit choice v' + this._lastStateVersion);
                    this._lastSyncRequestTime = now;
                    eventManager.emit('playerRequestsFullSync', {
                        fromPlayerId: this.myPlayerId,
                    });
     ...
     */
    onVariableChanged_networkedGameState({ newValue: newState, }) {
        if (!newState || newState.stateVersion === undefined)
            return;
    ...
    processStateUpdate(newState) {
        console.log('[CVM] processStateUpdate - state:', newState ? newState.state : 'undefined', 'version:', newState ? newState.stateVersion : 'undefined');
        ...
        if (newState.state === 'WAITING') {
        ...
        else if (newState.state === 'ACTIVE_GAME' ||
            ...
            if (!myHand && this._handPrimed) {
                ...
                if (!this._pilePrimed && this._handPrimed)
                    this._pilePrimed = true;
                if (this._pilePrimed) {
                    ...
                    var activeSuitArg = newState.activeSuit === null ? undefined : newState.activeSuit;
        ...
        if (newState.isAwaitingSuitChoice &&
    ...
    onSpriteClicked({ sprite }) {
        console.log('[CVM] onSpriteClicked - sprite:', sprite ? sprite.uniqueId : 'undefined', 'state:', this._localGameState ? this._localGameState.state : 'no state');
        if (sprite == undefined || !this._localGameState)
            return;
        // Help button toggle
        ...
        if (this._localGameState.state === 'WAITING') {
            if (sprite.uniqueId === 'start_game_button' &&
        ...
        if (this._localGameState.state === 'GAME_OVER') {
            console.log('[CVM] Click in GAME_OVER state - sprite.uniqueId:', sprite.uniqueId);
            if (sprite.uniqueId === 'play_again_button' &&
                this.myPlayerId === this._localGameState.hostPlayerId) {
                console.log('[CVM] Play Again button clicked! Emitting hostStartsGame');
                eventManager.emit('hostStartsGame', { fromPlayerId: this.myPlayerId });
            }
            else {
                console.log('[CVM] Play Again click check failed - uniqueId match:', sprite.uniqueId === 'play_again_button', 'isHost:', this.myPlayerId === this._localGameState.hostPlayerId);
            }
            return;
        }
        // In-game interactions
        if (this._localGameState.state === 'ACTIVE_GAME') {
            if (parseInt(this._localGameState.currentPlayerId) !== this.myPlayerId ||
    ...
    createTableBackground() {
        ...
        if (!spriteManager.getSprite(tableId)) {
            ...
            spriteManager.addSprite('cell', this._prune({
                uniqueId: tableId,
                positionX: 0,
                positionY: 0,
     ...
     */
    triggerWinnerConfetti(winnerId) {
        try {
    ...
    drawMyHand(allPlayerHands) {
        ...
        var idealHandWidth = this.cardWidth + (handSize - 1) * (this.cardWidth + buffer);
        ...
        var totalHandWidth = this.cardWidth + (handSize - 1) * (this.cardWidth + actualBuffer);
        var startX = (this.stageWidth - totalHandWidth) / 2;
        for (var i = 0; i < handSize; i++) {
            var card = myHand[i];
            if (!card || !card.assetId)
                continue;
            var xPos = startX + i * (this.cardWidth + actualBuffer);
            var yPos = this.stageHeight -
    ...
    drawSuitSelector() {
        ...
        for (var i = 0; i < suits.length; i++) {
            ...
            if (artDetails.suitIcon) {
                ...
                var visualSprite = spriteManager.addSprite(artDetails.suitIcon, this._prune({
                ...
                if (visualSprite)
                    this._suitSelectorIds.push(visualSprite.uniqueId);
                // 2. Create transparent clickable rectangle overlay
                var clickableOverlay = spriteManager.addSprite('rect', this._prune({
    ...
    _createTextWithBackground(config) {
        ...
        var bg = spriteManager.addSprite('rect', this._prune({
            uniqueId: bgId,
            positionX: bgX,
            positionY: bgY,
            width: bgWidth,
        ...
        // Create text on top
        var text = spriteManager.addSprite('text', this._prune({
            uniqueId: textId,
            text: config.text,
            positionX: textX,
            positionY: textY,
    ...
    _drawLobbyInstructions() {
        ...
        var startY = this.centerY + (this._layoutConfig.INSTRUCTIONS_Y_OFFSET || 80);
        ...
        var titleSprite = spriteManager.addSprite('text', this._prune({
            uniqueId: titleId,
            text: titleText,
            positionX: titleX,
            positionY: startY,
        ...
        if (titleSprite)
            this._instructionSpriteIds.push(titleId);
        ...
        for (var r = 0; r < instructions.rules.length; r++) {
            ...
            var ruleSprite = spriteManager.addSprite('text', this._prune({
                uniqueId: ruleId,
                text: ruleText,
            ...
            if (ruleSprite)
                this._instructionSpriteIds.push(ruleId);
        ...
        var headerSprite = spriteManager.addSprite('text', this._prune({
        ...
        if (headerSprite)
            this._instructionSpriteIds.push(specialHeaderId);
        currentY += lineHeight - 5;
        ...
        for (var s = 0; s < instructions.specialCardRules.length; s++) {
            ...
            var specialSprite = spriteManager.addSprite('text', this._prune({
                uniqueId: specialId,
                text: specialText,
                positionX: specialX,
                positionY: currentY,
            ...
            if (specialSprite)
                this._instructionSpriteIds.push(specialId);
    ...
    _drawPlayerCount(numPlayers) {
        ...
        var yPos = this.stageHeight * (this._layoutConfig.PLAYER_COUNT_Y_PERCENT || 0.03);
        var bgPaddingX = this._layoutConfig.TEXT_BG_PADDING_X;
        var bgPaddingY = this._layoutConfig.TEXT_BG_PADDING_Y;
        var bgWidth = textWidth + bgPaddingX * 2;
        // Center the box first, then position text inside with equal padding
        ...
        if (spriteManager.getSprite(countId)) {
            ...
            if (text)
                this.persistentUiIds.push(text.uniqueId);
        }
    }
    /**
     ...
     */
    drawPlayAgainButton(gameState) {
        console.log('[CVM] drawPlayAgainButton called - myPlayerId:', this.myPlayerId, 'hostPlayerId:', gameState.hostPlayerId);
        if (this.myPlayerId !== gameState.hostPlayerId) {
        ...
        if (buttonExists) {
            ...
            // Create background first
            console.log('[CVM] Creating Play Again button at x:', centeredX, 'y:', yPos);
            var bg = spriteManager.addSprite('rect', this._prune({
                uniqueId: buttonBgId,
                positionX: bgX,
                positionY: yPos - bgPaddingY,
            ...
            if (button) {
                console.log('[CVM] Play Again button created successfully:', button.uniqueId);
                this.persistentUiIds.push(button.uniqueId);
            }
            else {
     ...
     */
    updatePlayPileView(playPile, activeSuit) {
        if (activeSuit === null)
        ...
        if (currentAssetId === this._lastPileTopAssetId &&
        ...
        if (spriteManager.getSprite(baseId))
            spriteManager.removeSprite(baseId);
        if (spriteManager.getSprite(rankId))
            spriteManager.removeSprite(rankId);
        if (spriteManager.getSprite(suitId))
            spriteManager.removeSprite(suitId);
        if (spriteManager.getSprite(centerId))
            spriteManager.removeSprite(centerId);
        var xPos = this.centerX - this.cardWidth;
        var yPos = this.centerY / 2;
        spriteManager.addSprite('blank_card', this._prune({
            uniqueId: baseId,
            positionX: xPos,
            positionY: yPos,
            width: this.cardWidth,
        ...
        spriteManager.addSprite('text', this._prune({
            uniqueId: rankId,
            text: '' + topCard.rank,
        ...
        if (art && art.suitIcon && !isJoker) {
            spriteManager.addSprite(art.suitIcon, this._prune({
                uniqueId: suitId,
        ...
        if (centerAsset) {
            ...
            spriteManager.addSprite(centerAsset, this._prune({
                uniqueId: centerId,
    ...
    updateLastAction(gameState) {
        ...
        var turnIndicatorY = this.stageHeight * this._layoutConfig.TURN_INDICATOR_Y_PERCENT;
        ...
        if (existingSprite) {
            ...
            spriteManager.addSprite('rect', this._prune({
                uniqueId: bgId,
                positionX: bgX,
                positionY: yPos - bgPaddingY,
                width: bgWidth,
     ...
     */
    updateCampingTpZone(x, y, width, height) {
        if (!playerManager.isHost)
            return;
        if (!this._layoutConfig.CAMPING_TP_ENABLED)
            return;
        ...
        if (existing) {
            ...
            console.log('[CVM] Created camping TP zone at (' +
                zoneX +
                ', ' +
                zoneY +
                ') size ' +
    ...
    onSpriteCollisionStart({ sprite1, sprite2, }) {
        ...
        var playerSprite = s1.playerId !== undefined
            ? s1
            : s2.playerId !== undefined
                ? s2
                : null;
        var zoneSprite = s1.collisionGroup === 'camping_tp'
            ? s1
        ...
        if (playerSprite && zoneSprite) {
            ...
            if (pid) {
                ...
                var teleportRadius = this._layoutConfig.CAMPING_TP_TELEPORT_RADIUS || 150;
                ...
                console.log('[CVM] Teleported player P' +
                    pid +
    ...
    updateDrawPileView(deckSize) {
        ...
        if (deckSize > 0) {
            if (idx === -1) {
                var xPos = this.centerX + this.cardWidth;
                var yPos = this.centerY / 2;
                var createdIds = this.drawCardBack(pileId, xPos, yPos, this.cardWidth, this.cardHeight, 0, true);
     ...
     */
    updatePlayerStatus(allPlayerHands) {
        ...
        for (var pid in allPlayerHands) {
            if (allPlayerHands.hasOwnProperty(pid)) {
                ...
                console.log('[CVM] checking playerId:', playerId, 'vs myPlayerId:', this.myPlayerId, 'equal?', playerId === this.myPlayerId);
        ...
        var startY = this.stageHeight * this._layoutConfig.PLAYER_STATUS_Y_PERCENT - 20;
        ...
        for (var idx = 0; idx < allIds.length; idx++) {
            ...
            var statusText = playerName + ': ' + cardCount + (cardCount === 1 ? ' card' : ' cards');
    ...
    drawCardFace(config) {
        ...
        var base = spriteManager.addSprite('blank_card', this._prune({
        ...
        if (base)
            createdIds.push(base.uniqueId);
        var rankTop = spriteManager.addSprite('text', this._prune({
        ...
        if (rankTop)
            createdIds.push(rankTop.uniqueId);
        // Don't show suit icon for joker
        var isJoker = config.card && config.card.rank === 'joker';
        if (artDetails.suitIcon && !isJoker) {
            var suitTop = spriteManager.addSprite(artDetails.suitIcon, this._prune({
            ...
            if (suitTop)
                createdIds.push(suitTop.uniqueId);
        }
        ...
        if (centerArtAsset) {
            ...
            var faceYOffset = isFaceCard
                ...
                : 0;
            var centerArt = spriteManager.addSprite(centerArtAsset, this._prune({
                uniqueId: config.baseId + '_center',
                positionX: config.x + config.width / 2 - centerSize / 2,
                positionY: config.y + config.height / 2 - centerSize / 2 + faceYOffset,
                width: centerSize,
                height: centerSize,
                // scaleX: isJoker? .5: isFaceCard ? .2 : .5,
                // scaleY: isJoker? .5: isFaceCard ? .2 : .5,
            ...
            if (centerArt)
                createdIds.push(centerArt.uniqueId);
        }
        ...
        if (specialLabel) {
            ...
            var labelStrokeColor = this._layoutConfig.SPECIAL_CARD_LABEL_STROKE_COLOR || '#FFFFFF';
            var labelStrokeWeight = this._layoutConfig.SPECIAL_CARD_LABEL_STROKE_WEIGHT || 2;
            ...
            var labelSprite = spriteManager.addSprite('text', this._prune({
            ...
            if (labelSprite)
                createdIds.push(labelSprite.uniqueId);
        }
        return createdIds;
    }
    /** Simple card back block. */
    drawCardBack(baseId, x, y, w, h, rotation, isInteractive) {
        var createdIds = [];
        console.log("draw pile interactive", isInteractive);
        // Base blank card
        var blankCard = spriteManager.addSprite('blank_card', this._prune({
            uniqueId: baseId,
            positionX: x,
            positionY: y,
            width: w,
        ...
        if (blankCard)
            createdIds.push(blankCard.uniqueId);
        // Card background sprite centered on top
        var cardBackground = spriteManager.addSprite('card_background', this._prune({
        ...
        if (cardBackground)
            createdIds.push(cardBackground.uniqueId);
        return createdIds;
    }
    /**
    ...
    _getSpecialCardLabel(rank) {
        ...
        if (special.skip && special.skip.enabled && rank === special.skip.rank) {
            console.log('[CVM] _getSpecialCardLabel: skip match for rank=' +
                rank +
                ' label=' +
                special.skip.label);
        ...
        if (special.reverse &&
            ...
            console.log('[CVM] _getSpecialCardLabel: reverse match for rank=' +
                rank +
                ' label=' +
        ...
        if (special.wild && special.wild.enabled && rank === special.wild.rank) {
            console.log('[CVM] _getSpecialCardLabel: wild match for rank=' +
                rank +
                ' label=' +
                special.wild.label);
