class CrazyEightsAnalyticsManager extends SystemScript {
  ...
  _turnCount: number; // Turns taken in current game
  _cardsPlayedCount: number; // Cards played (not drawn) in current game
  _maxHandSizes: PseudoMap; // Track max hand size per player
  _forcedDrawCount: number; // Times players had to draw (no playable card)
  _deckReshuffleCount: number; // Times deck was reshuffled from discard
   ...
   * @param playerCount - Number of players in game
   */
  trackGameCompletion(winnerId: number, playerCount: number) {
    if (!this.analyticsPublicKey) return;
    ...
    try {
      ...
      if (this.lastGameInitiatorProfileId) {
        analytics.push({
          analyticName:
            'crazyEightsGameInitiator-' + this.lastGameInitiatorProfileId,
        });
      }
      ...
      console.log(
        '[C8Analytics] Game completion tracked - Winner:',
        winnerId,
        'Turns:',
        this._turnCount,
      );
   ...
   */
  trackWildCardPlay(playerId: number, chosenSuit: string) {
    if (!this.analyticsPublicKey) return;
    ...
    try {
      ...
      console.log(
        '[C8Analytics] Wild card play tracked:',
        playerId,
        'chose',
        chosenSuit,
      );
   ...
   */
  trackDraw2Play(playerId: number) {
    if (!this.analyticsPublicKey) return;
   ...
   */
  trackReversePlay(playerId: number) {
    if (!this.analyticsPublicKey) return;
   ...
   */
  trackCardDraw(playerId: number, wasForcedDraw: boolean = true) {
    if (!this.analyticsPublicKey) return;
   ...
   */
  trackCardPlayed(playerId: number) {
   ...
   */
  trackHandSize(playerId: number, handSize: number) {
    const key = playerId + '';
   ...
   */
  trackGameAbandon(playerId: number, playerCount: number) {
    if (!this.analyticsPublicKey) return;
   ...
   */
  trackSpectatorJoin(playerId: number) {
    if (!this.analyticsPublicKey) return;
