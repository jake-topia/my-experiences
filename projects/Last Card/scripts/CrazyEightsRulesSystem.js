...
class CrazyEightsRulesSystem extends SystemScript {
    ...
    isValidPlay(playedCard, topCard, activeSuit) {
        ...
        if (activeSuit !== undefined && activeSuit !== null && activeSuit !== '') {
            ...
            if (!chosenSuit)
                return false; // malformed active suit
            if (pSuit === chosenSuit)
                return true;
            // rank match only makes sense if we actually have a top card
            if (topCard && '' + topCard.rank === pRank)
                return true;
            return false;
        }
     ...
     */
    canDrawCard(playerHand, topCard, activeSuit) {
        if (!playerHand || !playerHand.length)
            return true;
        for (var i = 0; i < playerHand.length; i++) {
            if (this.isValidPlay(playerHand[i], topCard, activeSuit))
                return false;
        }
        return true;
    }
