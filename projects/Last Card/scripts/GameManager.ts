class GameManager extends SystemScript {
  ...
  _configProvider: any;
  _rulesProvider: any;
  _analyticsManager: any;

  // ---------- Config ----------
  _config: any;

  // ---------- Host-only authoritative state ----------
  _deck: PseudoList; // draw pile
  _playerHands: PseudoMap; // { [playerId: string]: Card[] }
  _discardPile: PseudoList; // top is last element
  _playerTurnOrder: PseudoList; // array of playerIds
  _currentPlayerIndex: number;
  _spectators: PseudoList; // players who joined mid-game

  // ---------- Fields mirrored to clients ----------
  currentGameState: string; // 'WAITING_FOR_PLAYERS' | 'PREPARING_GAME' | 'ACTIVE_GAME' | 'GAME_OVER'
  hostPlayerId: number; // who can start the game
  _stateVersion: number; // monotonic; clients ignore old versions
  _activeSuit: any; // '', or one of 'hearts'|'diamonds'|'clubs'|'spades'
  _isAwaitingSuitChoice: boolean; // true after a wild card until suit chosen
  _lastSuitChoiceT: number; // last processed clientSuitChoice timestamp
  _playDirection: number; // 1 = clockwise, -1 = counter-clockwise (for reverse)
  _pendingDraw2: number; // stacking draw 2 count (0 = none pending)
  _lastAction: string; // Last action taken (for display to players)
  _cardPlayedThisTurn: boolean; // true after a card is played, cleared on turn advance
  _hasDrawnThisTurn: boolean; // true after a draw, cleared on turn advance

  // ---------- Player Colors ----------
  _playerColors: PseudoMap; // position -> player tint color hex

  // ---------- Engine-safe helpers: never pass null/undefined into engine APIs ----------
  _nzStr(v: any, fb: string = ''): string {
    return v === null || v === undefined ? fb : '' + v;
  }
  _nzNum(v: any, fb: number = 0): number {
    const n = +v;
    // eslint-disable-next-line no-restricted-globals
    return v === null || v === undefined || isNaN(n) ? fb : n;
  }
  _nzBool(v: any, fb: boolean = false): boolean {
    return v === null || v === undefined ? fb : !!v;
  }
  ...
  onInit() {
    ...
    if (playerManager.isHost) {
      ...
      this._deck = [] as any;
      this._playerHands = {} as any;
      this._discardPile = [] as any;
      this._playerTurnOrder = [] as any;
      this._currentPlayerIndex = 0;
      this._activeSuit = '';
      this._isAwaitingSuitChoice = false;
      this._lastSuitChoiceT = 0;
      this._spectators = [] as any;
  ...
  onPlayerJoined({ playerId }: { playerId: number }) {
    ...
    if (
      ...
      console.log(
        '[GM] Game in progress - P' + playerId + ' becomes spectator',
      );
  ...
  onPlayerLeft({ playerId }: { playerId: number }) {
    ...
    if (this.currentGameState === 'ACTIVE_GAME') {
      ...
      if (wasInGame && this._analyticsManager) {
        this._analyticsManager.trackGameAbandon(
          playerId,
          this._playerTurnOrder.length + 1,
        );
      }
  ...
  _getPlayerPosition(playerId: number): number {
    const ids = playerManager.getPlayerIds();
    for (let i = 0; i < ids.length; i++) {
      if (ids[i] === playerId) return i;
    }
    return 0;
  }

  _assignPlayerColorAndNameplate(playerId: number) {
  ...
  _updatePlayerNameplate(playerId: number, isCurrentTurn: boolean) {
  ...
  _setSpectatorNameplate(playerId: number) {
    const details = playerManager.getPlayerDetails(playerId);
    if (!details) return;
  ...
  _updateAllPlayerNameplates() {
    const currentPlayerId =
      this._playerTurnOrder.length > 0
  ...
  updateLobbyState() {
    ...
    const ids = playerManager.getPlayerIds();
    const canStart =
      ids.length >= this._config.minPlayers &&
      ids.length <= this._config.maxPlayers;

    const lobbyHands: Record<string, any[]> = {};
    for (let i = 0; i < ids.length; i++) {
      lobbyHands[ids[i] + ''] = [];
    }
  ...
  prepareAndDealNow() {
    ...
    if (initialCard) {
      ...
      console.log(
        '[GM] prep: no valid initial discard; deck=' + this._deck.length,
      );
    }
  ...
  updateNetworkedState(
    winnerId = 0,
    feedback?: { playerId: number; message: string },
  ) {
    if (!playerManager.isHost) return;
    this._stateVersion++;
    ...
    const ids = playerManager.getPlayerIds();
    const handsPlain: Record<string, any[]> = {};
    for (let i = 0; i < ids.length; i++) {
      const pid = ids[i] + '';
  ...
  onEvent_playerWantsToPlayCard({
    ...
    if (this.currentGameState !== 'ACTIVE_GAME' || this._isAwaitingSuitChoice)
      return;
    if (this._cardPlayedThisTurn) return;

    const currentPlayerId = this._playerTurnOrder[this._currentPlayerIndex];
    if (fromPlayerId !== currentPlayerId) return;
    ...
    const ok = this._rulesProvider.isValidPlay(
      playedCard,
      topCard,
      this._activeSuit || undefined,
    );
    if (!ok) {
    ...
    console.log(
      '[GM] play ok: top=' + playedCard.assetId + ' handSize=' + updatedHand.length,
    );
    ...
    if (this._rulesProvider.isWildCard(playedCard)) {
      ...
      var wildPlayerName =
        wildPlayerDetails && wildPlayerDetails.username
    ...
    if (specialCards.skip.enabled && cardRank === specialCards.skip.rank) {
      ...
      var skipPlayerName =
        skipPlayerDetails && skipPlayerDetails.username
    ...
    if (
      specialCards.reverse.enabled &&
      cardRank === specialCards.reverse.rank
    ) {
      console.log(
        '[GM] Reverse played by P' +
          fromPlayerId +
          ' - direction was ' +
          this._playDirection,
      );
      ...
      var revPlayerName =
        revPlayerDetails && revPlayerDetails.username
    ...
    var normPlayerName =
      normPlayerDetails && normPlayerDetails.username
  ...
  onEvent_playerSelectsSuit({
    ...
    if (!playerManager.isHost || !this._isAwaitingSuitChoice) return;
  ...
  onEvent_playerWantsToDrawCard({ fromPlayerId }: { fromPlayerId: number }) {
    console.log(`[GM] onEvent_playerWantsToDrawCard ${fromPlayerId}`)
    if (
      !playerManager.isHost ||
      this.currentGameState !== 'ACTIVE_GAME' ||
    ...
    const currentPlayerId = this._playerTurnOrder[this._currentPlayerIndex];
    if (fromPlayerId !== currentPlayerId) return  console.log("[GM] onEvent_playerWantsToDrawCard not current player");
    if (this._hasDrawnThisTurn) return console.log("[GM] onEvent_playerWantsToDrawCard already drew this turn");
    ...
    if (
      !this._rulesProvider.canDrawCard(
        hand,
        topCard,
        this._activeSuit || undefined,
      )
  ...
  _advanceTurn() {
    if (this._playerTurnOrder.length === 0) return;
   ...
   */
  _forcePlayerDraw(playerId: number, count: number) {
  ...
  // ---------- Resync ----------
  onEvent_playerRequestsFullSync({ fromPlayerId }: { fromPlayerId: number }) {
    if (!playerManager.isHost) return;
    console.log(
      '[GM] full sync requested by P' +
        fromPlayerId +
        ' (v' +
        this._stateVersion +
        ')',
    ...
    if (this.currentGameState === 'WAITING_FOR_PLAYERS')
      this.updateLobbyState();
    else this.updateNetworkedState();
  }
  ...
  dealCards(numCards: number) {
    ...
    for (let c = 0; c < numCards; c++) {
      for (let p = 0; p < ids.length; p++) {
        ...
        if (card) this._playerHands[ids[p] + ''].push(card);
      }
    }
