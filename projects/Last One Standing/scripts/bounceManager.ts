class bounceManager extends ComponentScript {
  ...
  hasInit: boolean;
  allPlayers: any;          // stores total amount of players when the game starts
  playersOut: PseudoList;   // stores all the players that are out of the game (not including queued)
  speed: number;            // speed of the threat 
  lastCollided: number;     // holds the id of the last collided eliminated player, or -1 if a wall was hit 
  winnerDeclared: boolean;  // blocks duplicate winner events once the game is over
  ...
  onPhysicsStep() {
    ...
    if (isTooFarX) this.sprite.velocity.x = this.getNewVelocity(isTooFarRight);
    if (isTooFarY) this.sprite.velocity.y = this.getNewVelocity(isTooFarDown);
  }
   ...
   */
  getNewVelocity(isNegative: boolean) {
    if (!playerManager.isHost) return;
    this.lastCollided = -1;
    ...
    if (isNegative) return -this.speed;
    return this.speed;
  }
  ...
  onSpriteCollisionStart({ collisionX, collisionY, sprite }) {
    ...
    if (!playerManager.isHost) return;
    if (!this.allPlayers.includes(sprite.playerId) && !(this.checkInList(sprite.playerId))) return;
  ...
  onEvent_playerOut({playerID, stillJoined}){
    ...
    if(stillJoined){
      ...
      if(this.lastCollided === playerID) this.lastCollided = -1;
  ...
  getRemainingPlayerIds() {
    ...
    for (const playerID of this.allPlayers) {
      if (connectedPlayerIds.indexOf(playerID) === -1) continue;
      if (remainingPlayerIds.indexOf(playerID) !== -1) continue;
  ...
  emitWinnerIfGameOver() {
    ...
    if (remainingPlayerIds.length > 1) return;
