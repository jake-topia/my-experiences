class arenaManager extends SystemScript {
  ...
  winnerRewardAnnouncementIds: string[];
  ...
  onInit() {
    ...
    this.winnerRewardAnnouncementIds = [];
  ...
  hideCenterFruitSprite() {
    ...
    if (!centerFruitSpriteId) return;
    if (!spriteManager.getSprite(centerFruitSpriteId)) return;
  ...
  positionCenterFruitSprite(spriteId: string, opacity: number) {
  ...
  showWinner(playerName: string) {
    ...
    this.setWinnerRewardAnnouncements([]);
  }
  ...
  hideWinner() {
    ...
    spriteManager.updateSprite("centerTitleText", { text: "", opacity: 0 });
    spriteManager.updateSprite("centerSubtitleText", { text: "", opacity: 0 });
    spriteManager.updateSprite("centerFooterText", { text: "", opacity: 0 });
    this.setWinnerRewardAnnouncements([]);
  }

  setWinnerRewardAnnouncements(lines: string[]) {
    if (!playerManager.isHost) return;

    const contentWidth = this.getCenterContentWidth();
    const contentX = this.panelX + this.getCenterContentPadding();
    const startY = this.panelY + 244;
    const lineSpacing = 24;

    for (let i = 0; i < lines.length; i++) {
      const announcementId = this.ensureWinnerRewardAnnouncementSprite(i);
      const truncatedText = this.truncateTextToFit(lines[i], contentWidth, 22);
      spriteManager.updateSprite(announcementId, {
        positionX: contentX,
        positionY: startY + i * lineSpacing,
        containerWidth: contentWidth,
        text: truncatedText,
        align: "center",
        fontSize: this.getFittedFontSize(truncatedText, contentWidth, 22, 16),
        fontWeight: "bold",
        fontColor: "#ffe082",
        strokeThickness: 3,
        strokeColor: "#000000",
        opacity: truncatedText ? 1 : 0,
      });
    }

    for (let i = lines.length; i < this.winnerRewardAnnouncementIds.length; i++) {
      spriteManager.updateSprite(this.winnerRewardAnnouncementIds[i], {
        text: "",
        opacity: 0,
      });
    }
  ...
  ensureWinnerRewardAnnouncementSprite(index: number): string {
    const announcementId = "winnerRewardAnnouncementText_" + index.toString();
    if (!spriteManager.getSprite(announcementId)) {
      spriteManager.addSprite("basicText", {
        uniqueId: announcementId,
        positionX: this.panelX,
        positionY: this.panelY,
        containerWidth: this.panelSize,
        align: "center",
        text: "",
        fontSize: 22,
        fontWeight: "bold",
    ...
    if (index < this.winnerRewardAnnouncementIds.length) {
      this.winnerRewardAnnouncementIds[index] = announcementId;
    } else {
      this.winnerRewardAnnouncementIds.push(announcementId);
    }
