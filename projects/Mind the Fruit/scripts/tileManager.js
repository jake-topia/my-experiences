...
class tileManager extends SystemScript {
    ...
    onInit() {
        ...
        const arenaSize = this.rows * this.tileSize + (this.rows - 1) * this.gap;
        this.arenaTop = this.worldHeight - arenaSize - arenaLeftGap;
    ...
    buildTiles() {
        ...
        for (let row = 0; row < this.rows; row++) {
            for (let col = 0; col < this.cols; col++) {
                if (!this.shouldCreateTile(row, col))
                    continue;
    ...
    prepareRoundFromFruitOptions(fruitOptions) {
        if (!playerManager.isHost)
            return;
        ...
        const safeFruitName = fruitOptions[Math.floor(Math.random() * fruitOptions.length)];
        const safeTileMap = {};
        ...
        stateManager.setVariable("roundFruitOptionsMap", this.arrayToIndexedMap(fruitOptions));
    ...
    buildRoundFruitOptions(roundNumber) {
        const options = [];
    ...
    getRoundFruitPoolSize(roundNumber) {
        if (roundNumber <= 1)
            return 3;
        if (roundNumber <= 3)
            return 4;
    ...
    setTileFruitDisplay(tileId, fruitName, shouldShow) {
        if (!playerManager.isHost)
            return;
        const tileData = this.tileDataById[tileId];
        if (!tileData)
    ...
    ensureFruitSprite(tileData, fruitName) {
        if (!playerManager.isHost)
            return false;
    ...
    removeTileFruitSprite(tileData, exceptSpriteId) {
        if (!playerManager.isHost)
            return;
        const activeFruitSpriteId = this.getActiveFruitSpriteId(tileData);
        if (activeFruitSpriteId &&
    ...
    getFruitSpriteUniqueId(tileData, fruitName) {
    ...
    getActiveFruitSpriteId(tileData) {
        if (tileData.activeFruitSpriteId)
            return tileData.activeFruitSpriteId;
        if (tileData.currentFruitSpriteName) {
            return this.getFruitSpriteUniqueId(tileData, tileData.currentFruitSpriteName);
        }
    ...
    refreshPlayerCurrentTileMap(playerIds) {
        if (!playerManager.isHost)
            return;
    ...
    findTileUnderPlayer(playerId) {
        ...
        for (let i = 0; i < this.tileIds.length; i++) {
            ...
            if (!tileSprite || tileSprite.opacity <= 0)
                continue;
            if (footX >= tileSprite.position.x &&
    ...
    onSpriteCollisionStart({ sprite1, sprite2, }) {
        if (!playerManager.isHost)
            return;
        if (!this.shouldTrackPlayerTiles())
            return;
    ...
    decodePlayerTileCollision(sprite1, sprite2) {
