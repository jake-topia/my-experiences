...
class animator extends SystemScript {
    ...
    onInit() {
        ...
        this.startTextAnimation = timerManager.animate({
            ...
            playbackEase: "Linear",
            onLoop: () => {
                const periodCount = this.countTrailingPeriods(this.startText.text);
                if (this.startTimeLeft <= 0) {
                    return;
                }
    ...
    onEvent_playerCompleted() {
        if (!playerManager.isHost || this.firstFinish)
            return;
    ...
    onEvent_movingForward({ playerId }) {
        ...
        const playerData = utils.getPlayerData(playerAssignment);
