class gameManager extends SystemScript {
  ...
  getInteractivePublicKey() {
    const interactivePublicKey = stateManager.getVariable(
  ...
  sendAnalytics(analytics: PseudoList) {
    try {
  ...
  async setWorldActivity(type: string) {
    try {
      if (!playerManager.isHost) return;
      const interactivePublicKey = this.getInteractivePublicKey();
      console.log("Key is : ", interactivePublicKey);
      if (!interactivePublicKey) return;
