...
class main extends SystemScript {
    ...
    onPlayerJoined({ playerId }) {
        if (!playerManager.isHost)
            return;
        this.logJoinGameAnalytic(playerId);
    ...
    onHostStart() {
        ...
        const spots = this.layoutGrid(this.experienceWidth, this.experienceHeight, this.tileW, this.tileH, this.gap, this.totalTiles, this.rows, this.tilesPerRow);
        let it = 0;
        for (const { x, y } of spots) {
            let idString = "t" + it.toString();
            let rowNum = Math.floor(it / this.tilesPerRow);
            let colNum = it % this.tilesPerRow;
        ...
        this.winnerText = spriteManager.addSprite("basicText", {
            text: "",
            align: "center", containerWidth: this.experienceWidth, positionY: this.experienceHeight / 2 - 125,
            fontSize: 50, fontWeight: "bold"
        });
        //     this.utils.makeText({ text: "                    ", align: "center", justify: "start" });
        ...
        this.logAnalyticPerPlayer("start");
    }
    idToNumber(id) {
    ...
    getValueAtId(id) {
        let tileNum = this.idToNumber(id);
        if (tileNum == null && tileNum !== 0)
            return false;
        let rowNum = Math.floor(tileNum / this.tilesPerRow);
        let colNum = tileNum % this.tilesPerRow;
    ...
    addWinnerAndReset(playerId) {
        ...
        this.logAnalyticPerPlayer("completion");
    }
    onEvent_reset() {
        this.logAnalyticPerPlayer("start");
        // clear winner text
        ...
        // state
        stateManager.setVariable("tileFadedMap", {});
        stateManager.setVariable("tileStanders", {});
        // reset tiles
    ...
    generatePath(tilesPerRow, rows) {
        ...
        let goodCount = 2;
        let idealGoodCount = 3;
        let currentRow = rows - 2;
        let currentCol = startTile;
        let previousCol = startTile;
        while (1) {
            if (currentRow === 0)
                break;
            let included = [];
            if (previousCol !== currentCol - 1 &&
    ...
    itemsThatFit(size, tile, gap, buffer = 0) {
        if (tile <= 0)
            return 0;
        if (size < tile)
            return 0;
    ...
    layoutGrid(areaW, areaH, tileW, tileH, gap, count, // total tiles to place
    rows, cols) {
        const perRow = cols;
        const perCol = rows;
    ...
    //
    profileIdHash;
    logAnalyticPerPlayer(analyticName) {
        if (!playerManager.isHost)
            return;
        const publicKey = stateManager.getVariable("publicKey");
        const players = playerManager.getPlayerIds();
        const analytics = [];
        if (analyticName === "completion") {
            analytics.push({ analyticName: `gamesOf${playerManager.getPlayerIds().length}` });
        }
        players.forEach(playerId => {
            let profileId = this.profileIdHash[String(playerId)];
            if (!profileId) {
                profileId = playerManager.getPlayerDetails(playerId)?.profileId;
                if (!profileId)
                    return;
                this.profileIdHash[String(playerId)] = profileId;
            }
            analytics.push({ analyticName, playerId, profileId, uniqueKey: profileId });
        });
    ...
    logJoinGameAnalytic(playerId) {
        ...
        analytics.push({ analyticName: "join", playerId, profileId, uniqueKey: profileId });
        integrationsManager.putPublicKeyAnalytics({
            interactivePublicKey: publicKey,
            analytics,
        });
