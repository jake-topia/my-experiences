class gameManager extends SystemScript {
  ...
  centralText: PseudoSprite;
  winnerText: PseudoSprite;

  timerAnim: any;
  timerAnimIsCounting: boolean;
  timerAnimTimeLeft: number;
  ...
  onInit() {
    ...
    this.centralText = spriteManager.addSprite("basicText",{
      positionY: this.experienceHeight / 2,
      fontSize: 50,
      containerWidth:this.experienceWidth,
      align:"center",
      text: "",
    })
  ...
  onPlayerJoined({ playerId }) {
    ...
    playerManager.setNameplate(
      playerId,
      "\uD83E\uDD76 " + playerManager.getPlayerDetails(playerId).username + " \uD83E\uDD76", // + '\uD83D\uDFE6' BLUE SQUARE
      // "\u2601\uFE0F " CLOUD
      // "\uD83E\uDDCA" ICE CUBE
      // "\u2744\uFE0F " SNOWFLAKE
    );
