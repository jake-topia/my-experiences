class tileManager extends SystemScript {
  ...
  fauxSprite: PseudoSprite;

  tileFadeAnim: any;
  toFadeOutTracker: any;
  toFadeInTracker: any;

  tileToFade: string; // tileId
  timeFading: number;
  maxStanders: number;
  ...
  onInit() {
    ...
    this.tileFadeAnim = timerManager.animate({
      ...
      onLoop: () => {
        ...
        if (!contender || !contender.tileId) return;
        ...
        if (this.countdownTimeLeft > 0) {
          if (contenderCount <= 1) this.countdownTimeLeft = this.COUNTDOWN_TIME;
  ...
  async fadeOut(): Promise<void> {
    if (stateManager.getVariable("winnerAnnounced") === true) return;
  ...
  async fadeIn(id: string): Promise<void> {
    if (stateManager.getVariable("winnerAnnounced") === true) return;
    ...
    timerManager.animate({
      ...
      onUpdate: () => {
        ...
        if (!isFadedMapReset && spriteManager.getSprite(id).opacity > 0.01) {
          // if wait 3 seconds is over
      ...
      onComplete: () => {
        ...
        this.fadeOutList = this.fadeOutList.filter((item: string) => item !== id);
      },
      onBegin: () => {
        // fix the detection of inside tiles with new collision system
        ...
        for (const hitbox of Object.keys(hitboxes)) {
          ...
          if (
            hitboxSprite.position.x < tileSprite.position.x + this.TILE_SIZE &&
  ...
  addTileStander(tileId: string) {
    const ts: Record<string, any> = stateManager.getVariable("tileStanders");
    ts[tileId] = Math.max(1, (ts[tileId] ?? 0) + 1);
    stateManager.setVariable("tileStanders", ts);
  }

  removeTileStander(tileId: string) {
    const ts: Record<string, any> = stateManager.getVariable("tileStanders");
    ts[tileId] = Math.max(0, (ts[tileId] ?? 0) - 1);
    stateManager.setVariable("tileStanders", ts);
  }

  getMaxStanders(exclude?: string[]): { tileId: string | null; maxStanders: number } {
    const ts: Record<string, any> = stateManager.getVariable("tileStanders");
    const excluded = exclude || [];

    const tileKeys = Object.keys(ts).filter((tileId) => {
      if (!this.isTile(tileId)) return false;
