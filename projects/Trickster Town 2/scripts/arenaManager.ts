class arenaManager extends SystemScript {
  ...
  lastSyncedNightWaitingText: string;
  ...
  onInit() {
    ...
    this.lastSyncedNightWaitingText = "";

    this.ensureBackgroundSprites();
    this.ensureSyncedCountdownSprite();
    this.ensureSyncedNightWaitingSprite();
  ...
  ensureSyncedNightWaitingSprite() {
    if (!playerManager.isHost) return;

    const waitingOptions: any = {
      uniqueId: "ui_night_waiting_synced",
      positionX: 150,
      positionY: 1440,
      containerWidth: 1200,
      align: "center",
      text: this.lastSyncedNightWaitingText || "",
      fontSize: 24,
      fontWeight: "bold",
      fontColor: this.countdownTextColor,
      isInteractive: false,
    ...
    if (!spriteManager.getSprite("ui_night_waiting_synced")) {
      spriteManager.addSprite("baseText", waitingOptions);
      return;
    }

    spriteManager.updateSprite("ui_night_waiting_synced", waitingOptions);
  }

  setSyncedNightWaitingText(text: string) {
    if (!playerManager.isHost) return;
    ...
    if (this.lastSyncedNightWaitingText === nextText) return;

    this.lastSyncedNightWaitingText = nextText;
    spriteManager.updateSprite("ui_night_waiting_synced", {
      text: nextText,
    });
  }
  ...
  teleportPlayerToSeat(playerId: number) {
    if (!playerManager.isHost) return;

    const seatMap = this.debugGetGlobalMap(
      "playerSeatMap",
      "arenaManager.teleportPlayerToSeat",
    );
    const seatData = seatMap ? seatMap[playerId.toString()] : null;
    if (!seatData) return;
  ...
  teleportPlayerToTrialCenter(playerId: number) {
    if (!playerManager.isHost) return;

    try {
      playerManager.teleportPlayers([playerId], {
        distributionType: "area",
        positionX: this.centerX - 2,
        positionY: this.centerY - 2,
        width: 4,
        height: 4,
      });
    } catch (e) {}
